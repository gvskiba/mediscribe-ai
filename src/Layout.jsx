...
export default function Layout({ children, currentPageName }) {
  ...
  return (
    <div className="bg-white flex flex-col h-screen">
      ...
      {/* Main Content */}
      {(() => {
        const darkPages = ['NoteDetail','ClinicalNoteStudio','Dashboard','Calendar','NotesLibrary','PatientDashboard','ClinicalGuidelines','Calculators','MedicalNews','AppSettings','Procedures','Shift','SoapCompiler','SoapCompilerStandalone','AntibioticStewardship','PediatricDosing','DrugReference','MedicationReference','CMELearningCenter','DischargePlanning','OrderSetBuilder','NoteTemplates','CustomTemplates','AddendumManager','LiveTranscription','CantMissDiagnoses','BillingDashboard','DrugsBugs','UserPreferences','DiagnosticStewardship','CommandCenter','NursingFlowsheet','AutoCoder','ClinicalDecisionSupport','KnowledgeBaseV2','NewPatientInput','PatientEducationGenerator','EDProcedureNotes','MedicalDecisionMaking','ERPlanBuilder','ERx'];
        const fullPagePages = [...darkPages, 'Results'];
        ...
        return (
          <main
            ...
            style={{ marginLeft: (showSidebar && !isHome && currentPageName !== 'MedicationReference' && currentPageName !== 'KnowledgeBaseV2' && currentPageName !== 'NewPatientInput' && currentPageName !== 'PatientEducationGenerator' && currentPageName !== 'EDProcedureNotes' && currentPageName !== 'MedicalDecisionMaking' && currentPageName !== 'ERPlanBuilder' && currentPageName !== 'ERx') ? 72 : 0, paddingTop: (currentPageName !== 'Home' && currentPageName !== 'ClinicalNoteStudio' && currentPageName !== 'NoteDetail' && currentPageName !== 'NoteTemplates' && currentPageName !== 'CustomTemplates' && currentPageName !== 'PatientDashboard' && currentPageName !== 'DischargePlanning' && currentPageName !== 'UserPreferences' && currentPageName !== 'DiagnosticStewardship' && currentPageName !== 'NoteCreationHub' && currentPageName !== 'Results' && currentPageName !== 'NursingFlowsheet' && currentPageName !== 'AutoCoder' && currentPageName !== 'ClinicalDecisionSupport' && currentPageName !== 'DrugReference' && currentPageName !== 'MedicationReference' && currentPageName !== 'KnowledgeBaseV2' && currentPageName !== 'NewPatientInput' && currentPageName !== 'PatientEducationGenerator' && currentPageName !== 'EDProcedureNotes' && currentPageName !== 'MedicalDecisionMaking' && currentPageName !== 'ERPlanBuilder' && currentPageName !== 'ERx') ? '48px' : '0', flex: 1, overflowY: 'auto' }}
          >
            <div className={isHome || fullPagePages.includes(currentPageName) ? 'w-full h-full' : 'p-4 md:p-8 max-w-7xl mx-auto'}>
              {children}
            </div>
            {/* Medical AI Chatbot */}
            <MedicalChatbot />
