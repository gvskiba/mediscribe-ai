...
export default function Layout({ children, currentPageName }) {
  ...
  return (
    <div className="min-h-screen bg-white flex flex-col">
      ...
      {/* Desktop Header */}
      <header className={`hidden lg:block fixed top-0 right-0 bg-white border-b border-slate-200 z-40 ${currentPageName === 'Home' ? '!hidden' : ''}`} style={{ left: showSidebar ? 64 : 0 }} style={{ background: 'linear-gradient(135deg, #f8fafc 0%, #ffffff 100%)' }}>
      ...
      {/* Mobile Header */}
      <div className={`lg:hidden fixed top-0 left-0 right-0 bg-white border-b border-slate-200 text-slate-900 px-4 py-3 z-40 space-y-3 ${currentPageName === 'Home' ? '!hidden' : ''}`} style={{ background: 'linear-gradient(135deg, #f8fafc 0%, #ffffff 100%)' }}>
