...
export function ruleBasedScan(meds, allergies) {
  ...
  if (has("amiodarone") && (has("atorvastatin") || has("statin") || has("simvastatin"))) findings.push({ type:"interaction", severity:"major", title:"Amiodarone + Statin (Myopathy)", drugs:["Amiodarone","Statin"], description:"Amiodarone inhibits CYP3A4, increasing statin blood levels and risk of myopathy/rhabdomyolysis.", mechanism:"CYP3A4 inhibition increases statin AUC.", recommendation:"Cap atorvastatin at 20mg/day with amiodarone. Avoid simvastatin >10mg. Switch to rosuvastatin if higher doses needed.", alternatives: { "Atorvastatin": findAlternatives("Atorvastatin", "Amiodarone", meds) } });
  if (has("lorazepam") && (has("morphine") || has("oxycodone") || has("fentanyl") || has("hydromorphone") || has("opioid"))) findings.push({ type:"interaction", severity:"critical", title:"Benzodiazepine + Opioid", drugs:["Lorazepam","Opioid"], description:"Combined CNS depression significantly increases risk of respiratory depression, sedation, and death. FDA black box warning.", mechanism:"Additive CNS/respiratory depression via GABA-A and mu-opioid receptors.", recommendation:"Avoid unless clearly indicated. Use lowest doses. Monitor O2 sat continuously. Have naloxone available.", alternatives: { "Lorazepam": findAlternatives("Lorazepam", "Morphine", meds) } });
  if (has("lithium") && (has("ibuprofen") || has("naproxen") || has("nsaid"))) findings.push({ type:"interaction", severity:"major", title:"Lithium + NSAID (Toxicity Risk)", drugs:["Lithium","NSAID"], description:"NSAIDs reduce renal lithium clearance, increasing levels and risk of toxicity.", mechanism:"NSAID inhibition of prostaglandins reduces renal lithium excretion.", recommendation:"Avoid NSAIDs with lithium. Use acetaminophen for pain. Monitor lithium levels if NSAID unavoidable.", alternatives: { "Ibuprofen": findAlternatives("Ibuprofen", "Lithium", meds) } });
