...
import {
  ...
  Loader2, Shield, Clock, ArrowLeft, Check, Baby, Syringe,
  Heart, Zap, BookOpen, Calculator
} from "lucide-react";
...
const SEPSIS_ANTIBIOTICS = [
  { id: "mod_sepsis_community", severity: "Moderate Sepsis — Community-Acquired (Unknown Source)", regimen: "Ceftriaxone 2 g IV q24h", additional: "Add azithromycin 500 mg IV if pneumonia suspected", notes: "Covers most Gram-positive and Gram-negative community organisms" },
  { id: "severe_sepsis_shock", severity: "Severe Sepsis / Septic Shock", regimen: "Piperacillin-Tazobactam 4.5 g IV q6–8h (extended infusion over 4 hr preferred)", additional: "Vancomycin 25–30 mg/kg IV loading dose if MRSA risk factors present", notes: "Broad-spectrum empiric; tailor urgently to culture results" },
  { id: "esbl_hap_risk", severity: "High ESBL Risk / HAP / Recent Antibiotics / Structural Lung Disease", regimen: "Meropenem 1–2 g IV q8h (extended infusion over 3 hr preferred)", additional: "Vancomycin 25–30 mg/kg IV load if MRSA risk", notes: "Reserve carbapenem for true ESBL/MDR risk; reassess and de-escalate at 48–72 hr" },
];

const SOURCE_ANTIBIOTICS = [
  { source: "Pneumonia (CAP)", regimen: "Ceftriaxone 1–2 g IV q24h + Azithromycin 500 mg IV q24h", alt: "Levofloxacin 750 mg IV q24h (PCN allergy or suspected atypical)", duration: "5–7 days (CAP); 7–14 days (HCAP/HAP)" },
  { source: "Urosepsis / Pyelonephritis", regimen: "Ceftriaxone 1–2 g IV q24h", alt: "Piperacillin-Tazobactam 3.375 g IV q6h if healthcare-associated or recent UTI", duration: "7–14 days depending on source control and organism" },
  { source: "Intra-abdominal Sepsis", regimen: "Piperacillin-Tazobactam 4.5 g IV q6h + urgent surgical or IR source control", alt: "Meropenem 1 g IV q8h + Metronidazole 500 mg IV q8h if PCN allergy", duration: "4–7 days if adequate source control achieved" },
  { source: "Skin / Soft Tissue (SSTI) / Necrotizing Fasciitis", regimen: "Vancomycin 25–30 mg/kg IV + Piperacillin-Tazobactam 4.5 g IV q6h", alt: "Daptomycin 6–10 mg/kg IV q24h if confirmed MRSA", duration: "Until clinically improved + source controlled; NF requires URGENT surgical debridement within hours" },
  { source: "Bacterial Meningitis", regimen: "Ceftriaxone 2 g IV q12h + Vancomycin 15 mg/kg q8–12h + Dexamethasone 0.15 mg/kg IV q6h × 4 days", alt: "Add Ampicillin 2 g IV q4h if Listeria risk (age > 50, immunocompromised, pregnancy)", duration: "7–21 days depending on organism" },
  { source: "Neutropenic Fever", regimen: "Piperacillin-Tazobactam 4.5 g IV q6h OR Cefepime 2 g IV q8h", alt: "Meropenem 1 g IV q8h if prior MDR or high-risk Pseudomonas; add Vancomycin if catheter-associated", duration: "Until ANC > 500 cells/μL and afebrile ≥ 48 hr minimum" },
];
...
const PREG_COLOR = { A: "bg-green-100 text-green-700", B: "bg-blue-100 text-blue-700", C: "bg-yellow-100 text-yellow-700", D: "bg-orange-100 text-orange-700", X: "bg-red-100 text-red-700" };

// ─── Helpers ──────────────────────────────────────────────────────────────────
function estimatePedWeight(ageYears) {
  if (ageYears <= 0.25) return 3.5 + (ageYears * 12 * 0.9);
  if (ageYears <= 1) return 6 + ((ageYears * 12 - 3) * 0.5);
  if (ageYears <= 2) return 10 + ((ageYears - 1) * 2.5);
  ...
  return (
    <button
      ...
      className={`flex-shrink-0 px-3 py-1.5 rounded-full text-xs font-semibold transition-all border ${
        active ? "text-white border-transparent shadow-sm" : "bg-transparent border-slate-200 text-slate-500 hover:border-slate-400"
      }`}
...
// ─── Med Card ──────────────────────────────────────────────────────────────────
function MedCard({ med, catColor, onAdd, pedWeightKg }) {
  const [open, setOpen] = useState(false);
  const line = LINE_STYLE[med.line] || LINE_STYLE.first;

  const pedDose = useMemo(() => {
    if (!pedWeightKg || !med.pediatric_dose?.mg_per_kg) return null;
    const raw = med.pediatric_dose.mg_per_kg * pedWeightKg;
    const capped = med.pediatric_dose.max_dose_mg ? Math.min(raw, med.pediatric_dose.max_dose_mg) : raw;
    return { dose: capped.toFixed(1), unit: med.pediatric_dose.unit, route: med.pediatric_dose.route, notes: med.pediatric_dose.notes };
  }, [pedWeightKg, med.pediatric_dose]);
      ...
      <div className="flex items-start gap-3 p-3 cursor-pointer" onClick={() => setOpen(v => !v)} style={{ borderLeft: `3px solid ${catColor}` }}>
        <div className="flex-1 min-w-0">
          <div className="flex flex-wrap items-center gap-1.5 mb-1">
            ...
            {med.pregnancy_category && <span className={`text-xs px-1.5 py-0.5 rounded font-mono ${PREG_COLOR[med.pregnancy_category] || "bg-slate-100 text-slate-600"}`}>Preg {med.pregnancy_category}</span>}
            {med.renal_adjustment && <span className="text-xs px-1.5 py-0.5 rounded bg-purple-100 text-purple-700">Renal ↓</span>}
          </div>
          ...
          {pedDose && <p className="text-xs text-blue-700 font-mono truncate mt-0.5"><span className="font-semibold">Ped ({pedWeightKg}kg):</span> {pedDose.dose} {pedDose.unit} {pedDose.route}</p>}
        </div>
        <div className="flex items-center gap-1 flex-shrink-0">
          <button onClick={e => { e.stopPropagation(); onAdd(med); }} className="p-1 rounded bg-blue-50 text-blue-600 hover:bg-blue-100 transition-colors" title="Add to note">
            ...
            <div className="px-4 pb-4 pt-2 border-t border-slate-100 space-y-3">
              ...
              {pedDose && (
                <div className="bg-blue-50 border border-blue-200 rounded-lg p-2.5">
                  <p className="text-xs font-bold text-blue-700 uppercase tracking-wide flex items-center gap-1 mb-1"><Baby className="w-3 h-3" /> Pediatric Dose ({pedWeightKg} kg)</p>
                  <p className="text-xs text-blue-800 font-mono"><strong>{pedDose.dose} {pedDose.unit}</strong> {pedDose.route}</p>
                  {pedDose.notes && <p className="text-xs text-blue-600 mt-0.5">{pedDose.notes}</p>}
                  {med.pediatric_dose.max_dose_mg && <p className="text-xs text-blue-500 mt-0.5">Max: {med.pediatric_dose.max_dose_mg} {med.pediatric_dose.unit}</p>}
                </div>
              )}
              ...
              {med.monitoring?.length > 0 && (
                <div>
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {med.monitoring.map((m, i) => <span key={i} className="text-xs bg-blue-50 text-blue-700 border border-blue-200 px-2 py-0.5 rounded-full">{m}</span>)}
                  </div>
                </div>
              )}
...
function MedReferenceTab({ onAddMed }) {
  ...
  const [showPedCalc, setShowPedCalc] = useState(false);

  const pedWeightKg = useMemo(() => {
    if (pedWeight) return parseFloat(pedWeight) || null;
    if (pedAge) {
      const est = estimatePedWeight(parseFloat(pedAge));
      return est > 0 ? parseFloat(est.toFixed(1)) : null;
    }
    return null;
  }, [pedWeight, pedAge]);
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex gap-2">
        ...
        <button
          onClick={() => setShowPedCalc(v => !v)}
          className={`flex items-center gap-1.5 px-3 h-8 rounded-lg text-xs font-semibold border transition-all flex-shrink-0 ${showPedCalc ? "bg-blue-600 text-white border-blue-600" : "border-slate-200 text-slate-600 hover:border-blue-300 hover:text-blue-600"}`}
        >
          <Baby className="w-3.5 h-3.5" /> Ped Dosing
        </button>
      </div>
        ...
        {showPedCalc && (
          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden">
            <div className="bg-blue-50 border border-blue-200 rounded-xl p-3 space-y-2">
              <p className="text-xs font-bold text-blue-700 flex items-center gap-1.5"><Calculator className="w-3.5 h-3.5" /> Pediatric Dose Calculator</p>
              <div className="flex gap-2">
                <div className="flex-1">
                  <label className="text-xs text-slate-500 mb-1 block">Weight (kg)</label>
                  <Input type="number" placeholder="e.g. 25" value={pedWeight} onChange={e => { setPedWeight(e.target.value); setPedAge(""); }} className="h-7 text-xs border-blue-200" />
                </div>
                <div className="flex-1">
                  <label className="text-xs text-slate-500 mb-1 block">Age (years, estimate)</label>
                  <Input type="number" placeholder="e.g. 5" value={pedAge} onChange={e => { setPedAge(e.target.value); setPedWeight(""); }} className="h-7 text-xs border-blue-200" />
                </div>
              </div>
              {pedWeightKg && <p className="text-xs text-blue-700 font-semibold">Using weight: <span className="font-bold">{pedWeightKg} kg</span>{pedAge && !pedWeight ? ` (estimated from age ${pedAge} yr)` : ""} — Broselow zone shown on each card</p>}
              <p className="text-xs text-blue-600 italic">Doses shown on expanded cards. Always use actual weight when available.</p>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <p className="text-xs text-slate-400">{filtered.length} medication{filtered.length !== 1 ? "s" : ""}{pedWeightKg ? ` · Ped dosing active (${pedWeightKg} kg)` : ""}</p>
      ...
      <div className="space-y-2 max-h-[calc(100vh-420px)] overflow-y-auto pr-1">
        {filtered.length === 0
          ...
          : filtered.map(med => {
              ...
              return <MedCard key={med.id} med={med} catColor={cat?.color || "#6b7280"} onAdd={onAddMed} pedWeightKg={pedWeightKg} />;
            })
        }
      </div>
    </div>
...
// ─── Sepsis Protocol Tab ───────────────────────────────────────────────────────
function SepsisProtocolTab({ onAddToNote }) {
  const [activeSection, setActiveSection] = useState("bundle");
  const sections = [
    { id: "bundle", label: "1-Hour Bundle" },
    { id: "empiric", label: "Empiric ABX" },
    { id: "source", label: "Source-Directed" },
  ];
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex items-start gap-2 px-3 py-2.5 rounded-lg bg-red-50 border border-red-200">
        ...
        <div>
          <p className="text-xs font-bold text-red-700">Sepsis 1-Hour Bundle — SSC 2018</p>
          <p className="text-xs text-red-600">All elements should be initiated within 1 hour of sepsis or septic shock recognition.</p>
        </div>
      </div>
      ...
      <div className="flex gap-1 bg-slate-100 p-1 rounded-lg">
        {sections.map(s => (
          <button key={s.id} onClick={() => setActiveSection(s.id)} className={`flex-1 py-1.5 rounded-md text-xs font-semibold transition-all ${activeSection === s.id ? "bg-white shadow-sm text-slate-900" : "text-slate-500 hover:text-slate-700"}`}>
            {s.label}
          </button>
        ))}
      </div>
      ...
      {activeSection === "bundle" && (
        <div className="space-y-2">
          {SEPSIS_BUNDLE_STEPS.map(step => (
            <div key={step.step} className={`rounded-xl border p-3 ${step.priority === "critical" ? "border-red-200 bg-red-50" : "border-amber-200 bg-amber-50"}`}>
              <div className="flex items-start gap-3">
                <div className={`w-6 h-6 rounded-full flex-shrink-0 flex items-center justify-center text-xs font-bold mt-0.5 ${step.priority === "critical" ? "bg-red-600 text-white" : "bg-amber-500 text-white"}`}>{step.step}</div>
                <div className="flex-1">
                  <p className={`text-xs font-bold ${step.priority === "critical" ? "text-red-700" : "text-amber-800"}`}>{step.action}</p>
                  <p className={`text-xs mt-0.5 ${step.priority === "critical" ? "text-red-600" : "text-amber-700"}`}>{step.detail}</p>
                </div>
                {step.priority === "critical" && <Badge className="bg-red-600 text-white text-xs flex-shrink-0">CRITICAL</Badge>}
              </div>
            </div>
          ))}
          ...
          <div className="bg-slate-50 border border-slate-200 rounded-xl p-3 space-y-2">
            <p className="text-xs font-bold text-slate-700 flex items-center gap-1.5"><FlaskConical className="w-3.5 h-3.5" /> Fluid Resuscitation (SMART Trial)</p>
            <div className="grid grid-cols-2 gap-2 text-xs">
              <div className="bg-white rounded-lg border border-slate-200 p-2">
                <p className="font-semibold text-emerald-700">LR (Preferred)</p>
                <p className="text-slate-600 mt-0.5">30 mL/kg; SMART trial: reduced AKI and 30-day mortality vs NS</p>
              </div>
              <div className="bg-white rounded-lg border border-slate-200 p-2">
                <p className="font-semibold text-slate-600">NS (Alternative)</p>
                <p className="text-slate-600 mt-0.5">30 mL/kg if LR unavailable; risk of hyperchloremic metabolic acidosis</p>
              </div>
            </div>
            <p className="text-xs text-slate-500">Reassess fluid responsiveness after each 500 mL bolus. Target MAP ≥ 65 mmHg.</p>
          </div>
        </div>
      )}
      ...
      {activeSection === "empiric" && (
        <div className="space-y-2">
          <p className="text-xs text-slate-500 px-1">Administer within 1 hour of sepsis/septic shock recognition. Obtain ≥ 2 blood culture sets first without delaying antibiotics > 45 min.</p>
          {SEPSIS_ANTIBIOTICS.map(ab => (
            <div key={ab.id} className="rounded-xl border border-slate-200 bg-white p-3 space-y-1.5">
              <p className="text-xs font-bold text-slate-800">{ab.severity}</p>
              <div className="bg-emerald-50 border border-emerald-200 rounded-lg px-2.5 py-1.5">
                <p className="text-xs font-semibold text-emerald-800">Primary: {ab.regimen}</p>
              </div>
              {ab.additional && <p className="text-xs text-slate-600 pl-1"><span className="font-semibold">Add-on:</span> {ab.additional}</p>}
              <p className="text-xs text-slate-500 italic">{ab.notes}</p>
              <button onClick={() => onAddToNote(`SEPSIS ANTIBIOTICS (${ab.severity})\n• ${ab.regimen}\n${ab.additional ? `• ${ab.additional}\n` : ""}Note: ${ab.notes}`)} className="text-xs text-blue-600 hover:text-blue-800 font-semibold flex items-center gap-1">
      ...
      {activeSection === "source" && (
        <div className="space-y-2">
          <p className="text-xs text-slate-500 px-1">Source-directed empiric therapy. De-escalate at 48–72 hr based on culture results.</p>
          {SOURCE_ANTIBIOTICS.map((ab, i) => (
            <div key={i} className="rounded-xl border border-slate-200 bg-white p-3 space-y-1.5">
              <p className="text-xs font-bold text-slate-800 flex items-center gap-1.5"><span className="w-2 h-2 rounded-full bg-emerald-500 inline-block" />{ab.source}</p>
              <div className="bg-emerald-50 border border-emerald-200 rounded-lg px-2.5 py-1.5">
                <p className="text-xs font-semibold text-emerald-800">{ab.regimen}</p>
              </div>
              {ab.alt && <p className="text-xs text-slate-600 pl-1"><span className="font-semibold">Alternative:</span> {ab.alt}</p>}
              <p className="text-xs text-slate-500"><span className="font-semibold">Duration:</span> {ab.duration}</p>
              <button onClick={() => onAddToNote(`ANTIBIOTICS — ${ab.source}\n• ${ab.regimen}\n${ab.alt ? `• Alt: ${ab.alt}\n` : ""}Duration: ${ab.duration}`)} className="text-xs text-blue-600 hover:text-blue-800 font-semibold flex items-center gap-1">
                <Plus className="w-3 h-3" /> Add to Plan
              </button>
            </div>
          ))}
          ...
          <div className="space-y-1.5">
            {note.medications.map((med, idx) => (
              <div key={idx} className="group flex items-center gap-3 px-3 py-2 rounded-lg border border-slate-100 hover:border-slate-200 hover:bg-slate-50 transition-all">
                ...
                <button onClick={() => handleRemove(idx)} className="opacity-0 group-hover:opacity-100 text-red-400 hover:text-red-600 transition-opacity p-0.5">
                  <X className="w-3 h-3" />
                </button>
              </div>
            ))}
...
export default function MedicationsTab({ note, noteId, queryClient, isFirstTab, isLastTab, handleBack, handleNext }) {
  ...
  const handleAddToPlan = async (text) => {
    const updated = (note.plan || "") + "\n\n" + text;
    await base44.entities.ClinicalNote.update(noteId, { plan: updated });
    queryClient.invalidateQueries({ queryKey: ["note", noteId] });
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-4 space-y-3">
      ...
      <div>
        ...
        <p className="text-xs text-slate-400 mt-0.5">ER reference, AI recommendations, sepsis protocol, and interaction checks</p>
      </div>
      ...
      {activeTab === "sepsis" && (
        <div className="space-y-3">
          <SepsisProtocolTab onAddToNote={handleAddToPlan} />
          <CurrentMedsPanel note={note} noteId={noteId} queryClient={queryClient} />
        </div>
      )}
