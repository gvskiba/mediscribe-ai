// FhirDataSync.jsx
// Full FHIR R4 data sync panel for the NPI workflow.
...
function ConflictRow({ label, existing, incoming, onKeep, onUse }) {
  return (
    <div style={{ padding:"9px 12px", borderRadius:8, background:T.up, border:`1px solid ${T.gold}33`, marginBottom:6 }}>
      <div style={{ fontFamily:"'JetBrains Mono',monospace", fontSize:9, color:T.gold, letterSpacing:"0.08em", textTransform:"uppercase", marginBottom:6 }}>
        {label} — conflict
      </div>
      <div style={{ display:"flex", gap:8, flexWrap:"wrap", alignItems:"flex-start" }}>
        <div style={{ flex:1, minWidth:120 }}>
          <div style={{ fontFamily:"'JetBrains Mono',monospace", fontSize:8, color:T.txt4, letterSpacing:"0.08em", marginBottom:3 }}>EXISTING</div>
          <div style={{ fontFamily:"'DM Sans',sans-serif", fontSize:11, color:T.txt3, lineHeight:1.5 }}>{existing || "\u2014"}</div>
          <button onClick={onKeep}
            style={{ marginTop:5, padding:"2px 10px", borderRadius:4, border:`1px solid ${T.teal}44`, background:"transparent", color:T.teal, fontFamily:"'DM Sans',sans-serif", fontSize:10, fontWeight:600, cursor:"pointer" }}>
            Keep
          </button>
        </div>
        <div style={{ flex:1, minWidth:120 }}>
          <div style={{ fontFamily:"'JetBrains Mono',monospace", fontSize:8, color:T.blue, letterSpacing:"0.08em", marginBottom:3 }}>FROM FHIR</div>
          <div style={{ fontFamily:"'DM Sans',sans-serif", fontSize:11, color:T.txt2, lineHeight:1.5 }}>{incoming}</div>
          <button onClick={onUse}
            style={{ marginTop:5, padding:"2px 10px", borderRadius:4, border:`1px solid ${T.blue}44`, background:"transparent", color:T.blue, fontFamily:"'DM Sans',sans-serif", fontSize:10, fontWeight:600, cursor:"pointer" }}>
            Use FHIR
          </button>
        </div>
      </div>
...
function ResourceTile({ icon, label, count, status, onPull, busy }) {
  ...
  return (
    <div style={{ padding:"10px 12px", borderRadius:10, background:T.up, border:`1px solid ${col === T.txt4 ? T.bd : col+"33"}`, display:"flex", alignItems:"center", gap:10 }}>
      ...
      <button onClick={onPull} disabled={busy}
        style={{ padding:"5px 12px", borderRadius:6, border:`1px solid ${T.teal}44`, background:`${T.teal}10`, color:busy ? T.txt4 : T.teal, fontFamily:"'DM Sans',sans-serif", fontSize:11, fontWeight:600, cursor:busy ? "not-allowed" : "pointer", flexShrink:0, transition:"all .15s" }}>
        {busy ? "\u29c9" : "Pull"}
      </button>
    </div>
  );
...
export default function FhirDataSync({
    ...
    try {
      ...
      fields.forEach(f => {
        ...
        if (!f.existing || f.existing === f.incoming) {
          ...
          addConflict({ ...f, setter: (val) => setDemo(prev => ({ ...prev, [f.field]:val })), value:f.incoming });
        }
      });
  ...
  const pullConditions = useCallback(async () => {
    ...
    try {
      ...
      // Map each condition name to a pmhSelected key (best-effort fuzzy match)
      const updates = {};
      parsed.forEach(name => {
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", background:T.bg, color:T.txt, fontFamily:"'DM Sans',sans-serif", height:"100%" }}>
        ...
        <div style={{ display:"flex", alignItems:"flex-start", justifyContent:"space-between", gap:12, flexWrap:"wrap", marginBottom:12 }}>
          ...
          <div style={{ display:"flex", gap:7, flexShrink:0 }}>
            <button onClick={() => setShowConfig(s => !s)}
              style={{ padding:"7px 14px", borderRadius:8, border:`1px solid ${fhirBase ? "rgba(0,229,192,.35)" : T.bd}`, background: fhirBase ? "rgba(0,229,192,.08)" : T.up, color: fhirBase ? T.teal : T.txt4, fontFamily:"'DM Sans',sans-serif", fontSize:11, fontWeight:600, cursor:"pointer", display:"flex", alignItems:"center", gap:5 }}>
              \u2699\uFE0F {showConfig ? "Hide Config" : "Config"}
            </button>
            <button onClick={pullAll} disabled={anyBusy || !base}
              style={{ padding:"7px 16px", borderRadius:8, border:"none", background: base ? "linear-gradient(135deg,#00e5c0,#00b4d8)" : "rgba(42,77,114,.3)", color: base ? "#050f1e" : T.txt4, fontFamily:"'DM Sans',sans-serif", fontSize:12, fontWeight:700, cursor: base && !anyBusy ? "pointer" : "not-allowed", transition:"all .15s" }}>
        ...
        {showConfig && (
          <div style={{ padding:"14px 16px", borderRadius:10, background:T.panel, border:`1px solid ${T.bd}`, marginBottom:0 }}>
            ...
            <input
              ...
              style={{ width:"100%", background:T.up, border:`1px solid ${T.bd}`, borderRadius:7, padding:"8px 12px", color:T.txt, fontFamily:"'DM Sans',sans-serif", fontSize:12, outline:"none", boxSizing:"border-box" }}
            />
      ...
      <div style={{ overflowY:"auto", flex:1, padding:"16px 22px 40px", display:"flex", flexDirection:"column", gap:10 }}>
        ...
        {/* No endpoint callout */}
        {!base && (
          <div style={{ padding:"12px 14px", borderRadius:9, background:"rgba(59,158,255,.06)", border:"1px solid rgba(59,158,255,.2)", fontFamily:"'DM Sans',sans-serif", fontSize:12, color:T.txt3, lineHeight:1.6, display:"flex", gap:9, alignItems:"flex-start" }}>
        ...
        {base && !syncedAny && (
          <div style={{ padding:"10px 14px", borderRadius:8, background:"rgba(42,77,114,.12)", border:`1px solid ${T.bd}`, fontFamily:"'DM Sans',sans-serif", fontSize:11, color:T.txt4, lineHeight:1.6 }}>
        ...
        {conflicts.length > 0 && (
          <div style={{ marginTop:6 }}>
            <div style={{ fontFamily:"'JetBrains Mono',monospace", fontSize:9, color:T.gold, letterSpacing:"1.5px", textTransform:"uppercase", marginBottom:10, display:"flex", alignItems:"center", gap:7 }}>
        ...
        {syncedAny && conflicts.length === 0 && (
          <div style={{ padding:"10px 14px", borderRadius:9, background:"rgba(0,229,192,.05)", border:"1px solid rgba(0,229,192,.2)", fontFamily:"'DM Sans',sans-serif", fontSize:11, color:T.teal, display:"flex", alignItems:"center", gap:9 }}>
            <span style={{ fontSize:15 }}>\u2713</span>
            FHIR sync complete \u2014 {Object.values(counts).reduce((a,b)=>a+b,0)} record{Object.values(counts).reduce((a,b)=>a+b,0)!==1?"s":""} imported across {Object.values(statuses).filter(s=>s==="ok").length} resource type{Object.values(statuses).filter(s=>s==="ok").length!==1?"s":""}.
          </div>
        )}
