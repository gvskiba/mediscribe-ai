...
import { MEDIA_CATEGORIES, BODY_REGIONS } from "@/components/npi/npiData";
  ...
  return (
    <>
      ...
      <div style={{ position:"fixed", top:0, right:0, bottom:0, width:460, zIndex:9985,
        ...
        display:"flex", flexDirection:"column", boxShadow:"-10px 0 48px rgba(0,0,0,0.65)" }}>
        ...
        <div style={{ padding:"12px 16px 10px", borderBottom:"1px solid rgba(26,53,85,0.5)",
          ...
          <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between" }}>
            <div style={{ display:"flex", alignItems:"center", gap:9 }}>
              ...
              <div>
                <div style={{ fontFamily:"'Playfair Display',serif", fontWeight:700, fontSize:14, color:"var(--npi-txt)" }}>Media Attachments</div>
        ...
        <div style={{ display:"flex", borderBottom:"1px solid rgba(26,53,85,0.45)", background:"rgba(5,12,24,0.7)", flexShrink:0 }}>
          {TABS.map(t => (
            <button key={t.id} onClick={() => setActiveTab(t.id)}
              ...
              <span>{t.label}</span>
              {t.badge > 0 && (
                <span style={{ position:"absolute", top:5, right:"18%", minWidth:14, height:14, borderRadius:7,
        ...
        <div style={{ flex:1, overflowY:"auto", padding:"14px 16px" }}>
            ...
            <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
              ...
              {!staged && (
                <div
                  ...
                  <div style={{ fontFamily:"'DM Sans',sans-serif", fontSize:13, fontWeight:600, color:"var(--npi-txt)", textAlign:"center" }}>
                    Drop image here or click to browse
                  </div>
                  <div style={{ fontFamily:"'DM Sans',sans-serif", fontSize:11, color:"var(--npi-txt4)", textAlign:"center" }}>
                    Photos, ECG strips, wound images &middot; JPEG, PNG, PDF &middot; max 15&thinsp;MB
                  </div>
                  <div style={{ fontFamily:"'JetBrains Mono',monospace", fontSize:9, padding:"4px 12px", borderRadius:5,
                    letterSpacing:1, border:"1px solid rgba(0,229,192,0.3)", color:"var(--npi-teal)",
              ...
              {staged && (
                <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
                  <div style={{ display:"flex", gap:12, alignItems:"flex-start", padding:"12px", borderRadius:10,
                    background:"rgba(14,37,68,0.6)", border:"1px solid rgba(26,53,85,0.5)" }}>
                    <img src={staged.dataUrl} alt="preview"
                      style={{ width:96, height:96, objectFit:"cover", borderRadius:8, border:"1px solid rgba(42,77,114,0.4)", flexShrink:0 }} />
                    <div style={{ flex:1, minWidth:0 }}>
                      <div style={{ fontFamily:"'DM Sans',sans-serif", fontSize:12, fontWeight:600, color:"var(--npi-txt)",
                        overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>
                        {staged.file.name}
                      </div>
                      <div style={{ fontFamily:"'JetBrains Mono',monospace", fontSize:9, color:"var(--npi-txt4)", marginTop:2 }}>
                        {formatSize(staged.file.size)} &middot; {staged.file.type.split("/")[1]?.toUpperCase()}
                      </div>
                      <button onClick={() => setStaged(null)}
                        style={{ marginTop:8, fontFamily:"'JetBrains Mono',monospace", fontSize:8, letterSpacing:1, textTransform:"uppercase",
                          padding:"3px 9px", borderRadius:4, cursor:"pointer",
                          border:"1px solid rgba(255,107,107,0.35)", background:"transparent", color:"#ff8a8a" }}>
                        Remove
                      </button>
                    </div>
                  </div>
                    ...
                    <div style={{ display:"flex", flexWrap:"wrap", gap:5 }}>
                      {MEDIA_CATEGORIES.map(cat => (
                        <button key={cat.label} onClick={() => setStagedCat(c => c === cat.label ? "" : cat.label)}
                          style={{ ...chipBase(stagedCat === cat.label, cat.col), display:"flex", alignItems:"center", gap:4 }}>
              ...
              {!staged && (
                <div style={{ display:"flex", flexWrap:"wrap", gap:5, paddingTop:4 }}>
                  {MEDIA_CATEGORIES.map(cat => (
                    <span key={cat.label} style={{ fontFamily:"'DM Sans',sans-serif", fontSize:10, color:"var(--npi-txt4)", display:"flex", alignItems:"center", gap:3 }}>
          ...
          {activeTab === "gallery" && (
            <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
              {attachments.length === 0 ? (
                <div style={{ textAlign:"center", padding:"36px 0", color:"var(--npi-txt4)", fontFamily:"'DM Sans',sans-serif", fontSize:12, fontStyle:"italic" }}>
                  No images attached yet &mdash; use the Attach tab to add clinical photos
                </div>
              ) : (
                <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10 }}>
                  {attachments.map((att, idx) => {
                    ...
                    return (
                      <div key={att.id}
                        style={{ borderRadius:10, overflow:"hidden", background:"rgba(14,37,68,0.7)",
                        ...
                        <div style={{ position:"relative", height:120 }}>
                          ...
                          {cat && (
                            <div style={{ position:"absolute", top:5, left:5, background:"rgba(5,14,28,0.82)",
                              borderRadius:20, padding:"2px 7px", display:"flex", alignItems:"center", gap:4 }}>
                              <span style={{ fontSize:10 }}>{cat.icon}</span>
                              <span style={{ fontFamily:"'DM Sans',sans-serif", fontSize:9, fontWeight:600, color:cat.col }}>{cat.label}</span>
                            </div>
                          )}
                          {att.aiAnalysis && (
                            <div style={{ position:"absolute", top:5, right:5, background:"rgba(59,158,255,0.85)",
                              borderRadius:4, padding:"2px 5px", fontFamily:"'JetBrains Mono',monospace", fontSize:7, color:"#fff", letterSpacing:.5 }}>
                              AI
                            </div>
                          )}
                        </div>
                        <div style={{ padding:"7px 9px" }}>
                          {att.region && <div style={{ fontFamily:"'JetBrains Mono',monospace", fontSize:8, color:"var(--npi-txt4)", marginBottom:2 }}>{att.region}</div>}
                          {att.caption ? (
                            <div style={{ fontFamily:"'DM Sans',sans-serif", fontSize:11, color:"var(--npi-txt3)", lineHeight:1.4,
                              overflow:"hidden", textOverflow:"ellipsis", display:"-webkit-box", WebkitLineClamp:2, WebkitBoxOrient:"vertical" }}>
                              {att.caption}
                            </div>
                          ) : (
                            <div style={{ fontFamily:"'DM Sans',sans-serif", fontSize:10, color:"var(--npi-txt4)", fontStyle:"italic" }}>No caption</div>
                          )}
                          <div style={{ fontFamily:"'JetBrains Mono',monospace", fontSize:7, color:"var(--npi-txt4)", marginTop:4 }}>
                            {att.author} &middot; {elapsed(att.ts)}
                          </div>
                        </div>
                      </div>
                    );
          ...
          {activeTab === "log" && (
            <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
              <div style={{ fontFamily:"'JetBrains Mono',monospace", fontSize:9, color:"var(--npi-txt4)", letterSpacing:1.5, textTransform:"uppercase" }}>Attachment Log</div>
              {attachments.length === 0 ? (
                <div style={{ textAlign:"center", padding:"28px 0", color:"var(--npi-txt4)", fontFamily:"'DM Sans',sans-serif", fontSize:12, fontStyle:"italic" }}>No attachments logged</div>
              ) : attachments.map((att, idx) => {
                const cat = catMeta(att.category);
                return (
                  <div key={att.id} style={{ display:"flex", gap:10, padding:"10px 12px", borderRadius:9,
                    background:"rgba(14,37,68,0.7)", border:"1px solid rgba(26,53,85,0.4)",
                    borderLeft:`3px solid ${cat ? cat.col : "rgba(42,77,114,0.6)"}`, cursor:"pointer" }}
                    onClick={() => { setLightboxIdx(idx); setEditCap(att.caption); setEditMode(false); setActiveTab("gallery"); }}>
                    <img src={att.dataUrl} alt={att.name}
                      style={{ width:44, height:44, objectFit:"cover", borderRadius:6, border:"1px solid rgba(42,77,114,0.4)", flexShrink:0 }} />
                    <div style={{ flex:1, minWidth:0 }}>
                      <div style={{ display:"flex", alignItems:"center", gap:6, marginBottom:3 }}>
                        {cat && <span style={{ fontSize:11 }}>{cat.icon}</span>}
                        <span style={{ fontFamily:"'DM Sans',sans-serif", fontSize:11, fontWeight:600, color: cat ? cat.col : "var(--npi-txt3)" }}>
                          {att.category || "Uncategorized"}
                        </span>
                        {att.region && <span style={{ fontFamily:"'JetBrains Mono',monospace", fontSize:8, color:"var(--npi-txt4)" }}>{att.region}</span>}
                      </div>
                      {att.caption && (
                        <div style={{ fontFamily:"'DM Sans',sans-serif", fontSize:11, color:"var(--npi-txt3)", overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>
                          {att.caption}
                        </div>
                      )}
                      <div style={{ fontFamily:"'JetBrains Mono',monospace", fontSize:7, color:"var(--npi-txt4)", marginTop:3 }}>
                        {att.author} &middot; {elapsed(att.ts)} &middot; {formatSize(att.size)}{att.aiAnalysis ? " &middot; AI analyzed" : ""}
                      </div>
                    </div>
                  </div>
                );
      ...
      {lightboxItem && (
        <div onClick={() => setLightboxIdx(null)}
          style={{ position:"fixed", inset:0, zIndex:9990, background:"rgba(2,6,14,0.92)",
            backdropFilter:"blur(6px)", display:"flex", alignItems:"center", justifyContent:"center", padding:20 }}>
          <div onClick={e => e.stopPropagation()}
            style={{ background:"#071525", border:"1px solid rgba(26,53,85,0.7)", borderRadius:14,
              width:"min(680px,92vw)", maxHeight:"90vh", display:"flex", flexDirection:"column",
            ...
            <div style={{ padding:"12px 16px", borderBottom:"1px solid rgba(26,53,85,0.5)",
              ...
              <div style={{ display:"flex", alignItems:"center", gap:8 }}>
                {catMeta(lightboxItem.category) && <span style={{ fontSize:16 }}>{catMeta(lightboxItem.category).icon}</span>}
                <div>
                  ...
                  <div style={{ fontFamily:"'JetBrains Mono',monospace", fontSize:8, color:"var(--npi-txt4)", marginTop:1 }}>
                    {lightboxItem.region && `${lightboxItem.region} \xb7 `}
                    {lightboxItem.author} &middot; {elapsed(lightboxItem.ts)} &middot; {formatSize(lightboxItem.size)}
                  </div>
                </div>
              </div>
              <div style={{ display:"flex", gap:8 }}>
                ...
                <button onClick={() => deleteAttachment(lightboxItem.id)}
                  style={{ padding:"4px 10px", borderRadius:6, border:"1px solid rgba(255,107,107,0.35)",
                    background:"transparent", color:"#ff8a8a", cursor:"pointer", fontFamily:"'DM Sans',sans-serif", fontSize:11 }}>
                  Delete
                </button>
                <button onClick={() => setLightboxIdx(null)}
                  style={{ width:26, height:26, borderRadius:13, border:"1px solid rgba(42,77,114,0.5)",
                    background:"rgba(14,37,68,0.7)", color:"var(--npi-txt4)", fontSize:12, cursor:"pointer",
                    display:"flex", alignItems:"center", justifyContent:"center" }}>
                  &#x2715;
                </button>
              </div>
            ...
            <div style={{ background:"#030810", display:"flex", alignItems:"center", justifyContent:"center",
            ...
            <div style={{ padding:"12px 16px 16px", overflowY:"auto", display:"flex", flexDirection:"column", gap:10 }}>
                ...
                {editMode ? (
                  <div style={{ display:"flex", gap:8 }}>
                    ...
                    <div style={{ display:"flex", flexDirection:"column", gap:5 }}>
                      ...
                      <button onClick={() => setEditMode(false)}
                        style={{ padding:"5px 10px", borderRadius:6, border:"1px solid rgba(42,77,114,0.4)",
                          background:"transparent", color:"var(--npi-txt4)", fontFamily:"'DM Sans',sans-serif", fontSize:11, cursor:"pointer" }}>
                        Cancel
                      </button>
                    </div>
                  </div>
                  ...
                  <div style={{ display:"flex", gap:8, alignItems:"flex-start" }}>
                    ...
                    <button onClick={() => { setEditCap(lightboxItem.caption); setEditMode(true); }}
                      style={{ fontFamily:"'JetBrains Mono',monospace", fontSize:8, letterSpacing:1, textTransform:"uppercase",
                        padding:"3px 8px", borderRadius:4, cursor:"pointer", flexShrink:0,
                        border:"1px solid rgba(42,77,114,0.4)", background:"transparent", color:"var(--npi-txt4)" }}>
                      Edit
                    </button>
                  </div>
                )}
              ...
              <div style={{ padding:"11px 13px", borderRadius:9, background:"rgba(59,158,255,0.06)", border:"1px solid rgba(59,158,255,0.2)" }}>
                <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:7 }}>
                  <div style={{ fontFamily:"'JetBrains Mono',monospace", fontSize:9, color:"#3b9eff", letterSpacing:1.5, textTransform:"uppercase" }}>
                    AI Documentation Draft
                  </div>
                  <button onClick={() => analyzeAttachment(lightboxItem)} disabled={analyzing === lightboxItem.id}
                    style={{ fontFamily:"'JetBrains Mono',monospace", fontSize:8, letterSpacing:1, textTransform:"uppercase",
                ...
                {lightboxItem.aiAnalysis ? (
                  <div style={{ fontFamily:"'DM Sans',sans-serif", fontSize:12, color:"var(--npi-txt2)", lineHeight:1.65 }}>
                    {lightboxItem.aiAnalysis}
                  </div>
                ) : (
                  <div style={{ fontFamily:"'DM Sans',sans-serif", fontSize:11, color:"var(--npi-txt4)", fontStyle:"italic" }}>
