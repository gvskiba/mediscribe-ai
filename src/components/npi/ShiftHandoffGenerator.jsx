...
function buildHandoffContext({ demo, cc, vitals, medications, allergies, pmhSelected,
  ...
  if (vitals?.temp) vs.push(`T ${vitals.temp}`);
  ...
  if (pmhArr.length) lines.push(`PMH: ${pmhArr.slice(0,6).join(", ")}`);
  ...
  lines.push(`Allergies: ${alls || "NKDA"}`);
  ...
  if (pendingItems?.length)
    lines.push(`Pending: ${pendingItems.map(p => `[${p.type}] ${p.text}${p.contingency ? " → If positive: " + p.contingency : ""}`).join("; ")}`);

  lines.push(`Illness Severity (I-PASS): ${severity || "stable"}`);
  ...
  const handleGenerate = useCallback(async () => {
    ...
    try {
      ...
      const data = await base44.integrations.Core.InvokeLLM({
        ...
        prompt: `You are generating a structured I-PASS emergency department shift handoff. Be clinically precise, actionable, and concise. Do not pad with unnecessary details.
...
Respond ONLY with valid JSON, no markdown fences:
{
  ...
  "critical_values": "Any lab, imaging, or vital sign thresholds that require immediate action — omit if none",
  "code_status": "Document code status and surrogate if known — omit if not documented"
}

Generate I-PASS handoff for:

${ctx}`,
        response_json_schema: {
          type:"object",
          properties:{
            i_illness_severity: { type:"string" },
            p_patient_summary:  { type:"string" },
            a_action_list:      { type:"string" },
            s_situation_awareness: { type:"string" },
            synthesis_note:     { type:"string" },
            critical_values:    { type:"string" },
      ...
      mdmState, consults, disposition, dispReason, esiLevel,
      providerName, doorTime, pendingItems, severity, onToast]);
  ...
  }, [result, demo, providerName, receivingDoc, esiLevel, doorTime, sevConfig, onToast]);
      ...
      <div style={{ display:"flex", alignItems:"center",
        ...
        <div style={{ flex:1 }}>
          <div style={{ fontFamily:"'Playfair Display',serif",
            fontWeight:700, fontSize:16, color:T.cyan }}>
            Shift Handoff Generator
          </div>
          <div style={{ fontFamily:"'DM Sans',sans-serif", fontSize:10,
            color:T.txt4, marginTop:1 }}>
            I-PASS framework · AI-assembled from encounter · Pending items tracker
          </div>
        </div>
        {result && (
      ...
      {!result && (
        <div>
            ...
            <div style={{ display:"flex", gap:6, flexWrap:"wrap", marginTop:6 }}>
              <div style={{ position:"relative", flexShrink:0 }}>
                ...
                <span style={{ position:"absolute", right:8, top:"50%",
                  ...
                  fontSize:8, pointerEvents:"none" }}>v</span>
              </div>
          ...
          <button onClick={handleGenerate} disabled={busy}
            ...
            {busy ? "Assembling handoff..." : "Generate I-PASS Handoff"}
          </button>
      ...
      {result && (
        <div>
          <div style={{ display:"flex", alignItems:"center", gap:10,
            ...
            <span style={{ fontSize:20 }}>{sevConfig?.icon}</span>
            <div>
              <div style={{ fontFamily:"'Playfair Display',serif",
                fontWeight:700, fontSize:14, color:sevConfig?.color||T.teal }}>
                {sevConfig?.label}
              </div>
