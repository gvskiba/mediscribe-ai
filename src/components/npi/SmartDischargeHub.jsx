    ...
    <div style={{ display:"flex", alignItems:"center", gap:8,
        ...
        {["continue","hold","discontinue"].map(s => (
          <button key={s} onClick={() => onStatus(s)}
            style={{ fontFamily:"'JetBrains Mono',monospace", fontSize:8,
              ...
              letterSpacing:1, textTransform:"uppercase", transition:"all .1s",
      ...
      <button onClick={onRemove}
        style={{ background:"none", border:"none", color:T.txt4,
          ...
          flexShrink:0, lineHeight:1 }}>x</button>
    </div>
  );
}
...
PMH: ${Object.keys(pmhSelected || {}).filter(k => pmhSelected[k]).join(", ") || "none documented"}
...
Respond ONLY with valid JSON, no markdown fences:
{
  ...
  const handleGenerate = useCallback(async () => {
    ...
    try {
      ...
      const raw = await base44.integrations.Core.InvokeLLM({
        ...
        response_json_schema: {
          ...
          properties: {
            ...
            home_care:          { type: "string" },
            medications:        { type: "string" },
            activity:           { type: "string" },
            diet:               { type: "string" },
            wound_care:         { type: "string" },
            return_precautions: { type: "string" },
      ...
      setAiErr("Error generating instructions: " + (e.message || "Please try again"));
    } finally {
      setBusy(false);
    }
  }, [buildPrompt]);
    ...
    <div style={{ fontFamily:"'DM Sans',sans-serif", color:T.txt }}>
      ...
      <Panel
        ...
        {reconMeds.length === 0 && (
          <div style={{ fontFamily:"'DM Sans',sans-serif", fontSize:11,
            ...
            No medications entered — add below as needed.
          </div>
        )}
        ...
        {(beersCount > 0 || conflictCount > 0) && (
          <div style={{ marginTop:10, padding:"7px 10px", borderRadius:7,
            ...
            {beersCount > 0 && `${beersCount} Beers Criteria medication${beersCount>1?"s":""} flagged for patient age 65+ — consider alternatives or document indication.`}
          </div>
        )}
      </Panel>
      ...
      <Panel
        ...
        <div>
          <div style={{ fontFamily:"'JetBrains Mono',monospace", fontSize:8,
            color:T.txt4, letterSpacing:1.3, textTransform:"uppercase",
            marginBottom:4 }}>Additional Notes</div>
          <input value={followupNotes} onChange={e => setFollowupNotes(e.target.value)}
            placeholder="e.g. Call for appointment, results pending at follow-up"
            style={{ width:"100%", padding:"7px 10px",
              background:"rgba(14,37,68,0.7)",
              border:"1px solid rgba(42,79,122,0.4)",
              borderRadius:7, outline:"none",
