...
const ADULT_NORMALS = Object.fromEntries(VITAL_FIELDS.filter(f => f.norm).map(f => [f.key, f.norm]));
...
export default function VitalsTab({ vitals, setVitals, avpu, setAvpu, o2del, setO2del, pain, setPain, triage, setTriage, onAdvance, onToast, registration, demo }) {
  ...
  // A/V/P/U — AVPU when not focused in input
  useEffect(() => {
    const h = (e) => {
  ...
  // 0–9 — pain scale when not in input
  useEffect(() => {
    const h = (e) => {
      if (['INPUT','TEXTAREA'].includes(e.target.tagName)) return;
      if (e.ctrlKey || e.metaKey) return;
  ...
  return (
    <div style={{ fontFamily:"'DM Sans',sans-serif", maxWidth:800, display:'flex', flexDirection:'column', gap:18 }}>
      ...
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between' }}>
        ...
        <div style={{ display:'flex', gap:8, alignItems:'center', flexWrap:'wrap' }}>
          ...
          <button
            ...
            Fill Normals <kbd style={{ fontFamily:"'JetBrains Mono',monospace", fontSize:9, background:'rgba(0,229,192,.15)', border:'1px solid rgba(0,229,192,.3)', borderRadius:3, padding:'0 4px' }}>Ctrl+N</kbd>
          </button>
          <button onClick={() => setVitals({})} style={{ padding:'5px 12px', borderRadius:7, background:'none', border:'1px solid rgba(255,255,255,.12)', color:'var(--npi-txt4)', fontSize:12, cursor:'pointer' }}>Clear</button>
          {onAdvance && <button onClick={onAdvance} style={{ padding:'5px 14px', borderRadius:7, background:'var(--npi-teal)', color:'#050f1e', border:'none', fontSize:12, fontWeight:600, cursor:'pointer' }}>Next: Meds →</button>}
        </div>
      </div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:16 }}>
        ...
        {/* Pain scale */}
        <div>
          <div style={FL}>Pain Scale <span style={{ fontWeight:400, textTransform:'none', letterSpacing:0, color:'#64748b' }}>(press 0–9)</span></div>
          ...
          {pain && <div style={{ fontSize:10, color:PAIN_COLOR(parseInt(pain)), fontFamily:"'JetBrains Mono',monospace", marginTop:4 }}>Pain: {pain}/9</div>}
        </div>

        {/* AVPU */}
        <div>
          <div style={FL}>AVPU <span style={{ fontWeight:400, textTransform:'none', letterSpacing:0, color:'#64748b' }}>(press A/V/P/U)</span></div>
            ...
            {O2_OPTS.map((opt, i) => {
              ...
              return (
                <button key={opt} onClick={() => setO2del(active ? '' : opt)}
                  ...
                  <span style={{ fontFamily:"'JetBrains Mono',monospace", fontSize:8, color:'var(--npi-blue)', opacity:.6 }}>{i+1}</span>{opt}
                </button>
              );
            })}
          </div>
      ...
      <div style={{ display:'flex', flexWrap:'wrap', gap:12, padding:'8px 12px', background:'rgba(14,37,68,.5)', border:'1px solid var(--npi-bd)', borderRadius:8 }}>
        {[['Enter','Next vital field'],['Ctrl+N','Fill adult normals'],['0–9','Pain scale (unfocused)'],['A/V/P/U','AVPU level (unfocused)']].map(([k,d]) => (
          <div key={k} style={{ display:'flex', alignItems:'center', gap:5, fontSize:11, color:'var(--npi-txt4)' }}>
            <kbd style={{ fontFamily:"'JetBrains Mono',monospace", fontSize:9, background:'var(--npi-up)', border:'1px solid var(--npi-bhi)', borderRadius:3, padding:'0 5px', color:'var(--npi-blue)' }}>{k}</kbd>{d}
          </div>
        ))}
      </div>
    </div>
