...
function DarkInput({ value, onChange, placeholder, type = "text", onKeyDown, style = {} }) {
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
      ...
      <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
        ...
        <button
          ...
          style={{
            padding: "8px 14px", borderRadius: 8, fontSize: 11.5, fontWeight: 700, cursor: !inputVal.trim() || status === "validating" ? "not-allowed" : "pointer",
            background: !inputVal.trim() || status === "validating" ? T.muted : `linear-gradient(135deg, ${color}, ${color}bb)`,
            border: "none", color: T.navy, fontFamily: "inherit", whiteSpace: "nowrap", opacity: !inputVal.trim() ? 0.5 : 1,
            display: "flex", alignItems: "center", gap: 5, transition: "all 0.15s"
          }}
        >
          {status === "validating" ? <><Loader2 size={12} style={{ animation: "spin 0.8s linear infinite" }} /> Validating…</> : "Validate & Save"}
        </button>
      </div>
      ...
      <div style={{ fontSize: 10.5, color: T.dim }}>
        Get a free token at{" "}
        <a href={getUrl} target="_blank" rel="noopener noreferrer" style={{ color: color, textDecoration: "none" }}
          onMouseEnter={e => e.target.style.textDecoration = "underline"}
          onMouseLeave={e => e.target.style.textDecoration = "none"}>{urlLabel}</a>
...
export default function AppSettings() {
  ...
  const handleSave = async () => { await saveMutation.mutateAsync(settings); };
    ...
    <div style={{ minHeight: "100vh", background: T.navy, color: T.text, fontFamily: "'DM Sans', -apple-system, sans-serif" }}>
      ...
      <div style={{ maxWidth: 860, margin: "0 auto", padding: "32px 24px", animation: "fadeUp 0.35s ease both" }}>
        ...
        <div style={{ marginBottom: 28 }}>
          ...
          <div style={{ fontSize: 12.5, color: T.dim }}>Configure your API keys, hospital information, and clinical staff</div>
        </div>

        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>

          {/* News API Keys */}
          <SectionCard icon={Newspaper} iconColor={T.teal} title="TheNewsAPI.com" subtitle="Medical News Feed — powers the Medical News page">
            <ApiKeySection
          ...
          <SectionCard icon={Globe} iconColor={T.purple} title="Webz.io News API" subtitle="Alternative news source — powers the Medical News page">
            <ApiKeySection
          ...
          <SectionCard icon={Building2} iconColor={T.amber} title="Hospital Information" subtitle="Used in the top bar and clinical notes">
            <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
              ...
              <div style={{ paddingTop: 6 }}>
                <button
                  ...
                  style={{
                    padding: "8px 20px", borderRadius: 8, fontSize: 12, fontWeight: 700, cursor: saveMutation.isPending ? "not-allowed" : "pointer",
                    background: saveSuccess ? `linear-gradient(135deg, ${T.green}, #27ae60)` : `linear-gradient(135deg, ${T.teal}, ${T.teal2})`,
                    border: "none", color: T.navy, fontFamily: "inherit", transition: "all 0.2s",
                    display: "flex", alignItems: "center", gap: 6
                  }}
                >
                  {saveMutation.isPending ? <><Loader2 size={13} style={{ animation: "spin 0.8s linear infinite" }} /> Saving…</> : saveSuccess ? <><Check size={13} /> Saved!</> : "Save Changes"}
                </button>
              </div>
            </div>
          </SectionCard>
          ...
          <SectionCard icon={Users} iconColor={T.teal} title="Attending Physicians" subtitle="Shown in the top bar when set as default">
            ...
            <div style={{ background: T.edge, border: `1px solid ${T.border}`, borderRadius: 10, padding: "14px 16px", marginBottom: 16 }}>
              ...
              <button
                ...
                style={{
                  ...
                  border: "none", color: T.navy, fontSize: 11.5, fontWeight: 700, cursor: newAttending.name.trim() ? "pointer" : "not-allowed",
                ...
                settings.attendings.map((a) => {
                  ...
                  return (
                    <div key={a.id} style={{
                      ...
                      padding: "10px 14px", background: isDefault ? "rgba(0,212,188,0.05)" : T.edge,
                      border: `1px solid ${isDefault ? "rgba(0,212,188,0.25)" : T.border}`, borderRadius: 9, transition: "all 0.15s"
                    }}>
