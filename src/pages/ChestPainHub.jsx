...
function GuidedWorkflow({
  ...
  if (current.type === "dispo") {
    ...
    return (
      <div className="cph-fade">
        <GuidedHeader step={8} total={GUIDED_STEPS.length}
        ...
        {rec && (
          <div style={{ padding:"18px", borderRadius:13, marginBottom:14,
            ...
            <div style={{ display:"flex", alignItems:"center",
              ...
              <div style={{ flex:1 }}>
                <div style={{ fontFamily:"'Playfair Display',serif",
                  fontWeight:900, fontSize:24, color:rec.color }}>{rec.dispo}</div>
                <div style={{ fontFamily:"'DM Sans',sans-serif",
                  fontSize:11, color:T.txt3, marginTop:3 }}>{rec.detail}</div>
              </div>
              <button onClick={handleCopy}
            ...
            {rec.plan.map((p, i) => (
              <div key={i} style={{ display:"flex", gap:8,
                ...
                <span style={{ fontFamily:"'JetBrains Mono',monospace",
                  fontSize:10, color:rec.color, minWidth:20,
                  marginTop:2, fontWeight:700 }}>{i+1}.</span>
                <span style={{ fontFamily:"'DM Sans',sans-serif",
                  fontSize:13, color:T.txt2, lineHeight:1.6 }}>{p}</span>
              </div>
            ))}
          </div>
        )}
