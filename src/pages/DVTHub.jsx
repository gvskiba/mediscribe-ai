// DVTHub.jsx — DVT / VTE Management Hub
//
// Clinical basis:
//   - Wells DVT Score (Wells 1997, Wells 2003)
//   - ISTH major bleeding criteria
//   - DOAC selection (2023 ESC/ACC guidelines)
//   - RE-LY, ROCKET-AF, ARISTOTLE, ENGAGE AF — major DOAC trials
//   - HOKUSAI-VTE, AMPLIFY — DOAC in VTE
//   - Renal dosing: CrCl-based adjustments per FDA labeling
//
// Route: /DVTHub
    ...
    <button onClick={onToggle}
      ...
      {pts !== undefined && (
        <span style={{ fontFamily:"'JetBrains Mono',monospace", fontSize:11,
          fontWeight:700, color:color||T.blue, flexShrink:0 }}>+{pts}</span>
      )}
    </button>
  );
}
...
function WellsTab() {
  ...
  const score = WELLS_DVT_ITEMS.reduce((s, i) => s + (items[i.key] ? i.pts : 0), 0);
  const anySet = Object.keys(items).length > 0;

  const strata = score <= 0
    ? { label:"Low Probability",      color:T.teal,   prev:"< 5%",
        rec:"D-dimer — negative rules out DVT. If d-dimer positive or not available: bilateral proximal compression US." }
    : score <= 2
  ...
  return (
    <div className="dvt-in">
      <Card color={T.blue} title="Wells DVT Score (Modified, 2003)">
        <div style={{ display:"flex", alignItems:"center",
          ...
          <div style={{ fontFamily:"'DM Sans',sans-serif", fontSize:10,
            color:T.txt4 }}>Wells 1997, modified 2003. Validated in primary care and ED.</div>
          <div style={{ fontFamily:"'Playfair Display',serif",
            fontSize:52, fontWeight:900,
            color:anySet ? strata.color : T.txt4, lineHeight:1 }}>{score}</div>
        </div>
        {WELLS_DVT_ITEMS.map(item => (
        ...
        {anySet && (
          <div style={{ marginTop:8, padding:"10px 13px", borderRadius:9,
            ...
            <div style={{ fontFamily:"'Playfair Display',serif",
              fontWeight:700, fontSize:17, color:strata.color, marginBottom:4 }}>
      ...
      <Card color={T.teal} title="DVT Diagnosis Algorithm">
        {[
          { label:"Proximal DVT (popliteal, femoral, iliac)", color:T.coral,
            note:"Treat with anticoagulation. Consider IVC filter only if anticoagulation absolutely contraindicated." },
          { label:"Distal DVT (calf veins)", color:T.gold,
            note:"Anticoagulation if: symptoms, risk factors for proximal extension, or bilateral. Observation with serial US in 7 days if low risk and no symptoms." },
          { label:"Incidental DVT (on CT/MRI for other reason)", color:T.blue,
            note:"Treat same as symptomatic if proximal. Distal: use clinical judgment." },
          { label:"Upper extremity DVT", color:T.purple,
            note:"Less common but PE risk is real. Treat same as lower extremity DVT — DOACs are reasonable." },
        ].map((item, i) => (
...
// ═══════════════════════════════════════════════════════════════════════════
// TAB 2 — ANTICOAGULATION SELECTION
// ═══════════════════════════════════════════════════════════════════════════
const DOAC_DATA = [
  {
    ...
    halflife:"5–9h",
    renal:"Avoid if CrCl < 15 mL/min. Reduce monitoring if CrCl < 30.",
  ...
  },
  {
    ...
    halflife:"12h",
    renal:"Avoid if CrCl < 15. Dose reduce: 2 criteria of [age ≥80, wt ≤60 kg, Cr ≥1.5] → 2.5 mg BID",
  ...
  },
  {
    ...
    halflife:"12–17h",
    renal:"Avoid if CrCl < 30 for VTE. Dose 75 mg BID if CrCl 15–30 for AF.",
    ...
    pearls:"Only DOAC with a specific reversal agent. Requires 5-day bridging — less convenient for outpatient VTE. Higher GI bleed risk.",
  },
  {
    ...
    pearls:"HOKUSAI-VTE trial. Requires 5-day parenteral bridging. Less used in ED due to unique CrCl >95 caveat.",
  },
];
    ...
    <div className="dvt-in">
      ...
      {doac && (
        <div>
          {[
            ...
            <div key={f.label} style={{ padding:"8px 11px", borderRadius:8,
              ...
              <div style={{ fontFamily:"'JetBrains Mono',monospace",
                fontSize:8, color:f.color, letterSpacing:1.3,
          ...
          <div style={{ padding:"9px 12px", borderRadius:9,
            ...
            <div style={{ fontFamily:"'JetBrains Mono',monospace",
              fontSize:8, color:doac.color, letterSpacing:1.3,
              textTransform:"uppercase", marginBottom:4 }}>💎 Pearl</div>
            <div style={{ fontFamily:"'DM Sans',sans-serif",
      ...
      <Card color={T.orange} title="LMWH — Enoxaparin (When DOAC Not Appropriate)">
        {[
          ...
          "Reversal: protamine sulfate 1 mg per 1 mg enoxaparin (given within 8h) — 60% effective",
      ...
      <Card color={T.blue} title="VTE Treatment Duration">
        {[
          ...
          { cause:"Antiphospholipid syndrome + VTE", dur:"Indefinite — warfarin preferred (DOACs inferior per TRAPS)", color:T.red },
        ].map((d, i) => (
    ...
    return [
      {
        ...
        rec: ccl < 15 ? { status:"AVOID", note:"CrCl < 15 — avoid. No dose adjustment defined." }
          ...
          : { status:"Standard", note:"Standard dosing. 20 mg QD (VTE) or 15 mg BID × 21d then 20 mg QD." },
      },
      {
        drug:"Apixaban", color:"#4da6ff",
        rec: ccl < 15 ? { status:"AVOID", note:"CrCl < 15 — avoid." }
          : ccl < 30  ? { status:"Reduced", note:"10→5 mg BID VTE. AF dose: use standard with extra caution — or consider warfarin." }
      ...
      },
      {
        ...
        rec: ccl < 30 ? { status:"AVOID (VTE)", note:"CrCl < 30: avoid for VTE treatment. For AF: 75 mg BID if CrCl 15–30." }
          : ccl < 50  ? { status:"Caution", note:"CrCl 30–50: use with caution. Higher dabigatran exposure. CrCl 30–50 + P-gp inhibitor → reduce to 75 mg BID for AF." }
          : { status:"Standard", note:"Standard 150 mg BID. Most renally eliminated DOAC (~80%)." },
      },
      {
        drug:"Edoxaban", color:"#3dffa0",
        rec: ccl > 95 ? { status:"AVOID", note:"CrCl > 95 mL/min: AVOID — paradoxically less effective (increased clearance reduces exposure)." }
          : ccl < 15  ? { status:"AVOID", note:"CrCl < 15: avoid." }
          : ccl <= 50 ? { status:"Reduced", note:`CrCl ${ccl < 30 ? "15–30" : "30–50"}: 30 mg QD (reduced from standard 60 mg QD).` }
          : { status:"Standard", note:"60 mg QD after 5–10 days LMWH bridging." },
      },
      {
        drug:"Enoxaparin (LMWH)", color:T.orange,
        rec: ccl < 30 ? { status:"Reduced", note:"CrCl < 30: 1 mg/kg SQ q24h (once daily). Check anti-Xa trough (target 0.5–1.0 IU/mL)." }
          : { status:"Standard", note:"1 mg/kg SQ q12h or 1.5 mg/kg q24h. No dose adjustment for CrCl ≥ 30." },
      },
    ];
  }, [ccl]);

  const statusColor = s =>
    s === "Standard" ? T.teal : s === "Reduced" ? T.gold : s === "AVOID" || s === "AVOID (VTE)" ? T.red : T.orange;

  return (
    <div className="dvt-in">
      <Card color={T.gold} title="Creatinine Clearance (CrCl) — Enter for Dosing Guidance">
        <div style={{ fontFamily:"'DM Sans',sans-serif", fontSize:10,
          color:T.txt4, marginBottom:8 }}>
          Use Cockcroft-Gault equation with actual body weight (or adjusted if obese).
        </div>
        ...
        <div style={{ fontFamily:"'JetBrains Mono',monospace",
          fontSize:8, color:T.txt4, marginTop:5 }}>
          CG: [(140-age) × weight(kg)] / [72 × Cr(mg/dL)] × 0.85 if female
        </div>
      </Card>
        ...
        <div key={r.drug} style={{ padding:"10px 12px", borderRadius:9,
          ...
          <div style={{ display:"flex", alignItems:"center",
            ...
            <span style={{ fontFamily:"'JetBrains Mono',monospace",
              ...
              borderRadius:6, color:statusColor(r.rec.status),
  ...
  const populations = [
    {
      ...
      points:[
        ...
        "Warfarin: AVOID first trimester (teratogenic weeks 6–12) and near delivery. Safe second trimester.",
        "Postpartum: warfarin or LMWH safe with breastfeeding. DOACs probably safe but limited data.",
    ...
    },
    {
      ...
      points:[
        "Cancer-associated VTE: DOAC (rivaroxaban or edoxaban) OR LMWH — comparable efficacy per SELECT-D and HOKUSAI-Cancer",
        "DOACs in cancer: slight increase in GI bleeds with rivaroxaban/edoxaban — caution in GI malignancy",
        "Apixaban alternative for luminal GI cancer: lower GI bleed signal",
    ...
    },
    {
      ...
      points:[
        ...
        "Single-positive APS with first provoked VTE: unclear — discuss with hematology",
        "Catastrophic APS: anticoagulation + steroids + IVIG ± plasma exchange",
      ],
    },
    {
      label:"IVC Filter Indications", icon:"🔧", color:T.blue,
      points:[
        "Absolute: acute VTE + absolute contraindication to anticoagulation (active major bleeding)",
        "Relative: recurrent VTE despite therapeutic anticoagulation; massive PE with right heart strain",
        "NOT indicated: prophylactic without DVT/PE diagnosis, as supplement to anticoagulation",
        "Retrievable filters preferred — set plan for retrieval when anticoagulation resumable",
        "Complication rate: filter thrombosis, DVT, post-thrombotic syndrome, migration",
        "Resume anticoagulation as soon as safely possible — filter does NOT substitute for anticoagulation",
      ],
    },
    {
      label:"Bleeding on Anticoagulation", icon:"🩸", color:T.coral,
      points:[
        "Major bleeding: life-threatening, > 2g/dL Hgb drop, requires transfusion, critical site (CNS, retroperitoneal)",
        "Stop anticoagulant, supportive care (IVF, transfusion PRN)",
        "Rivaroxaban/apixaban/edoxaban: andexanet alfa IV (150 mg/min × 15–30 min + 120 mg/hr × 2h)",
        "Dabigatran: idarucizumab 5g IV (two 2.5g vials) — immediate and complete reversal",
        "LMWH: protamine 1 mg per 1 mg enoxaparin (up to 50 mg) — partial reversal if > 8h",
