...
For contraindications, list only ABSOLUTE contraindications.
...
For return_precautions, list specific symptoms that warrant returning to the ED.`,
  ...
  const fullCardText = card && !card._err ? [
    ...
    "",
    card.absolute_contraindications?.length ? `DO NOT USE IF:\n${card.absolute_contraindications.map(c=>`  • ${c}`).join("\n")}` : "",
    card.key_interactions?.length ? `DRUG INTERACTIONS:\n${card.key_interactions.map(i=>`  • ${i}`).join("\n")}` : "",
    card.avoid_with?.length ? `AVOID:\n${card.avoid_with.map(a=>`  • ${a}`).join("\n")}` : "",
    card.side_effects_watch?.length ? `WATCH FOR:\n${card.side_effects_watch.map(s=>`  • ${s}`).join("\n")}` : "",
    card.return_precautions?.length ? `RETURN TO ED IF:\n${card.return_precautions.map(r=>`  • ${r}`).join("\n")}` : "",
  ...
  return(
    <div style={{background:T.bg,minHeight:"100vh",padding:"20px 18px 60px",fontFamily:"DM Sans,sans-serif",color:T.txt}}>
        ...
        <div>
          ...
          <div style={{...gl({padding:"14px 16px",marginBottom:14})}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:10}}>
              {[["Age","ptAge",ptAge,setPtAge,"yr"],["Weight","ptWt",ptWt,setPtWt,"kg"],["CrCl","crcl",crcl,setCrcl,"mL/min"]].map(([l,k,v,set,u])=>(
                <div key={k}>
        ...
        <div style={{position:"sticky",top:16}}>
          {!card&&!loading&&(
            <div style={{textAlign:"center",padding:"60px 20px",color:T.dim}}>
              ...
              <div style={{fontSize:11,lineHeight:1.7,color:T.dim}}>
                {mode==="discharge"
          ...
          {loading&&(
            <div style={{...gl({padding:"40px",textAlign:"center"})}}>
              <Sp/>
              <div style={{fontSize:12,color:T.mut,marginTop:12}} className="drc-in">Analyzing drug data...</div>
            </div>
          )}
      ...
      <div style={{marginTop:40,textAlign:"center"}}>
        <span style={{fontFamily:"JetBrains Mono,monospace",fontSize:9,color:T.dim,letterSpacing:1.5}}>NOTRYA · DISCHARGE RX CARD · AI-GENERATED · VERIFY BEFORE PRESCRIBING</span>
      </div>
    </div>
  );
}