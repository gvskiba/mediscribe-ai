...
function SkeletonCard() {
  return (
    <div style={{
      ...glass, height:175, borderRadius:14,
...
function QuickBtn({ icon, label, count, color, onClick }) {
  ...
  return (
    <button onClick={onClick} style={{
      ...
      border:`1px solid ${c}2e`, background:`${c}0e`, color:c,
      transition:"all .12s",
    }}>
      ...
      {count > 0 && (
        <span style={{
          background:c, color:"#050f1e", borderRadius:20, padding:"1px 5px",
          fontFamily:"JetBrains Mono", fontSize:8, fontWeight:700, lineHeight:"14px",
        }}>{count}</span>
      )}
    </button>
  );
    ...
    <div className={`${PREFIX}-fade`} style={{
      ...
      {patient.status === "critical" && (
        <div className={`${PREFIX}-pulse`} style={{
          ...
          background:`radial-gradient(ellipse at 20% 20%,${T.red}0b 0%,transparent 65%)`,
        }}/>
      )}
        ...
        <div style={{ display:"flex", flexDirection:"column", alignItems:"flex-end", gap:3, flexShrink:0 }}>
          ...
          {patient.noteDraft && (
            <span style={{
              ...
              background:`${T.orange}13`, border:`1px solid ${T.orange}36`,
              color:T.orange,
            }}>NOTE DRAFT</span>
          )}
        </div>
        ...
        <div style={{ borderTop:"1px solid rgba(42,79,122,0.28)", padding:"5px 12px", display:"flex", flexWrap:"wrap", gap:4 }}>
          {pend.map(o => (
            <span key={o.id} style={{
              fontFamily:"JetBrains Mono", fontSize:7, fontWeight:700,
              padding:"2px 7px", borderRadius:20,
              background:`${T.gold}0f`, border:`1px solid ${T.gold}30`, color:T.gold,
            }}>
      ...
      <div style={{ display:"flex", gap:8 }}>
        ...
        {patient.noteDraft && (
          <button onClick={() => onSignNote(patient.id)} style={{
            flex:1, fontFamily:"DM Sans", fontWeight:700, fontSize:12,
            padding:"10px", borderRadius:9, cursor:"pointer",
            border:`1px solid ${T.green}40`, background:`${T.green}10`, color:T.green,
          }}>✅ Sign Note</button>
        )}
      </div>
    </div>
...
function OrdersPanel({ patient, onToast, onAddOrder }) {
