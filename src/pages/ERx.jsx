...
function PedsDoseCalculator({ drug, defaultWeight, defaultAgeMonths, onUseDose }) {
  ...
  const ageYears  = ageMonths ? `${Math.floor(ageMonths/12)} yr ${Math.floor(ageMonths%12)} mo` : null;
...
function PDMPQueryPanel() {
  ...
  const handleQuery = async () => {
    ...
    try {
      ...
      setQueryError(err?.message || "Unexpected error — please retry");
    } finally {
      setQuerying(false); // Fix 6: always clears spinner even if await throws
    }
  };
