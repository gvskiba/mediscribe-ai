    ...
    <div style={{ display: "flex", height: "100%", gap: 0 }}>
      ...
      <div style={{ flex: "0 0 340px", background: T.panel, borderRight: `1px solid rgba(26,53,85,0.6)`, display: "flex", flexDirection: "column", overflow: "hidden", color: T.txt, fontFamily: "DM Sans" }}>
        ...
        <div style={{ padding: "24px 18px", borderBottom: `1px solid rgba(26,53,85,0.6)`, background: `linear-gradient(135deg,${T.card}99,${T.panel}99)` }}>
          ...
          <input placeholder="🔍 Search medications..." value={searchQ} onChange={e => setSearchQ(e.target.value)}
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "8px", display: "flex", flexDirection: "column", gap: 4 }}>
          {isLoading ? (
            <div style={{ padding: "40px 20px", textAlign: "center", color: T.txt3 }}>
              ...
              <div style={{ fontSize: 12 }}>Loading medications...</div>
            </div>
          ) : filtered.length === 0 ? (
            <div style={{ padding: "40px 20px", textAlign: "center", color: T.txt3 }}>
              <div style={{ fontSize: 24, marginBottom: 8 }}>🔍</div>
              <div style={{ fontSize: 12 }}>No medications found</div>
            </div>
          ) : (
            filtered.map(d => {
              const isActive = selectedDrugId === d.id;
              return (
                ...
                >
                  ...
                  <div style={{ fontSize: 10, color: isActive ? "rgba(59,158,255,0.6)" : T.txt3, marginTop: 3, paddingTop: 6, borderTop: `1px solid ${isActive ? "rgba(59,158,255,0.2)" : "rgba(42,77,114,0.1)"}` }}>{d.category}</div>
                </button>
              );
            })
          )}
        ...
        {!selectedDrug ? (
          <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", textAlign: "center", zIndex: 1, position: "relative" }}>
            <div>
              ...
              <div style={{ fontSize: 13, color: T.txt3, maxWidth: 300 }}>Choose a drug from the sidebar to view pediatric dosing, interactions, and PDMP query tools.</div>
            </div>
          </div>
        ) : (
          <div style={{ flex: 1, overflowY: "auto", padding: "32px 40px", zIndex: 1, position: "relative" }}>
            <div style={{ maxWidth: 1000 }}>
              <button onClick={() => setSelectedDrugId(null)}
