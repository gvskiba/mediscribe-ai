...
const CC_DATA = {
  ...
  flank_pain: {
    icon:"🫁", label:"Flank Pain / Renal Colic", color:"#f59e0b",
    gl:"rgba(245,158,11,0.12)", cat:"Renal/GU",
    onset: ["sudden","acute","colicky","gradual","after trauma","spontaneous"],
    quality: ["severe colicky","sharp","stabbing","aching","constant","crampy"],
    location: ["right flank","left flank","bilateral flank","costovertebral angle (CVA)","radiating to groin"],
    severity: null,
    radiation: ["non-radiating","groin","lower abdomen","genitalia","inner thigh","bilateral groin"],
    duration: ["minutes","< 1 hr","1–6 hrs","6–24 hrs","days","recurrent episodes"],
    timing: ["constant","intermittent / colicky","episodic","progressive","waxing and waning"],
    worse: ["movement","activity","urination","lying still"],
    better: ["movement/walking","analgesics","IV fluids","NSAIDs"],
    assoc: ["hematuria","dysuria","nausea","vomiting","diaphoresis","tachycardia","restlessness","CVA tenderness","fever"],
    neg: ["no fever","no hematuria","no dysuria","no CVA tenderness","no pyuria"],
    riskFlags: { onset:"sudden", msg:"⚠ Sudden flank pain — R/O kidney stone, AAA, pyelonephritis — imaging indicated" },
  },
  weakness: {
    icon:"💪", label:"Weakness / Paralysis", color:"#8b5cf6",
    gl:"rgba(139,92,246,0.12)", cat:"Neuro",
    onset: ["sudden","acute","gradual","subacute","insidious","upon waking","after exertion"],
    quality: ["complete paralysis","partial weakness","flaccid","rigid","focal","generalized","ascending paralysis","inability to move limb"],
    location: ["right-sided weakness","left-sided weakness","bilateral lower extremities","bilateral upper extremities","focal (arm/leg)","ascending (lower to upper)","facial droop"],
    severity: null,
    radiation: ["N/A — focal neuro deficit"],
    duration: ["minutes","hours","onset during sleep","days","progressive"],
    timing: ["sudden","progressive","intermittent","constant","fluctuating"],
    worse: ["activity","exertion","heat (myasthenia)","end of day (myasthenia)"],
    better: ["rest","tPA (if stroke)","cool environment (myasthenia)"],
    assoc: ["speech difficulty","facial droop","diplopia","headache","altered consciousness","recent illness","tick exposure","paresthesia","numbness"],
    neg: ["no facial droop","no headache","no diplopia","no speech changes","no sensory loss","no altered consciousness"],
    riskFlags: { onset:"sudden", msg:"⚠ Sudden focal weakness — activate stroke protocol, NIHSS, STAT CT/CTA" },
  },
  cough: {
    icon:"🫁", label:"Cough", color:"#06b6d4",
    gl:"rgba(6,182,212,0.12)", cat:"Pulmonary",
    onset: ["sudden","gradual","acute viral URI","subacute","insidious","after aspiration","triggered by cold air"],
    quality: ["dry","productive","wet","wheezing","barky / croupy","whooping","hemoptysis","blood-tinged","paroxysmal"],
    location: ["N/A — lower respiratory"],
    severity: null,
    radiation: ["N/A"],
    duration: ["hours","days","1–2 weeks","weeks to months","chronic (> 3 weeks)"],
    timing: ["constant","intermittent","nocturnal","upon exertion","upon lying flat","paroxysmal at night"],
    worse: ["cold air","exertion","lying flat","breathing","talking","aspiration risk"],
    better: ["cough suppressants","bronchodilators","sitting upright","humidity","honey"],
    assoc: ["dyspnea","chest pain","wheezing","fever","chills","sputum production","hemoptysis","GI reflux","post-nasal drip"],
    neg: ["no hemoptysis","no dyspnea","no fever","no chest pain","no wheezing","no weight loss"],
    riskFlags: { qual:["hemoptysis","blood-tinged"], msg:"⚠ Hemoptysis with cough — R/O PE, pneumonia, malignancy — CXR indicated" },
  },
  hemoptysis: {
    icon:"🩸", label:"Hemoptysis (Coughing Up Blood)", color:"#ef4444",
    gl:"rgba(239,68,68,0.12)", cat:"Pulmonary",
    onset: ["sudden","acute","first episode","recurrent","with coughing fit"],
    quality: ["blood-tinged sputum","frank blood","clots","large volume","small streaks","massive hemoptysis"],
    location: ["N/A — pulmonary"],
    severity: null,
    radiation: ["N/A"],
    duration: ["< 1 ml","1–5 ml","5–30 ml","> 30 ml (life-threatening)","single event","recurrent"],
    timing: ["single episode","intermittent","with cough","progressive"],
    worse: ["coughing","exertion","anticoagulant use"],
    better: ["urgent evaluation","hemostasis interventions"],
    assoc: ["cough","dyspnea","chest pain","fever","recent URI","anticoagulation history","smoking history","malignancy history","TB exposure"],
    neg: ["no dyspnea","no chest pain","no fever","no weight loss","no prior episodes"],
    riskFlags: { qual:["frank blood","massive hemoptysis"], msg:"⚠ HEMOPTYSIS — R/O PE, pneumonia, malignancy, TB — STAT CXR, CBC, coagulation studies" },
  },
  anaphylaxis: {
    icon:"⚠️", label:"Anaphylaxis", color:"#dc2626",
    gl:"rgba(220,38,38,0.12)", cat:"Allergy/Critical",
    onset: ["immediate (< 5 min)","within 30 min","1–2 hrs after exposure","biphasic (delayed reaction)","unknown trigger"],
    quality: ["respiratory distress","stridor","wheezing","throat tightness","dysphagia","facial/tongue swelling","angioedema","cardiovascular collapse","syncope","urticaria"],
    location: ["face/lips/tongue","throat","generalized","systemic"],
    severity: null,
    radiation: ["N/A — systemic"],
    duration: ["< 30 min","30 min–2 hrs","hours","biphasic (recurrence)"],
    timing: ["constant","improving","worsening","biphasic pattern"],
    worse: ["further allergen exposure","exertion","upright position"],
    better: ["epinephrine IM","antihistamines","corticosteroids","supine position","oxygen"],
    assoc: ["dyspnea","wheezing","stridor","chest tightness","hypotension","syncope","nausea","vomiting","abdominal pain","diaphoresis","tachycardia","urticaria"],
    neg: ["no stridor","no dyspnea","no hypotension","no throat swelling","no prior reactions"],
    riskFlags: { assoc:["stridor","dyspnea","hypotension"], msg:"⚠ ANAPHYLAXIS — administer epinephrine IM 0.3–0.5 mg immediately, establish IV access, oxygen, monitoring" },
  },
  intoxication: {
    icon:"🍺", label:"Intoxication / Overdose", color:"#a16207",
    gl:"rgba(161,98,7,0.12)", cat:"Toxicology",
    onset: ["acute","sudden","after ingestion / use","after injection","unknown time"],
    quality: ["altered consciousness","slurred speech","ataxia","confusion","agitation","combativeness","sedation","stupor","respiratory depression","seizure","coma"],
    location: ["N/A — systemic"],
    severity: null,
    radiation: ["N/A"],
    duration: ["< 1 hr","1–4 hrs","4–12 hrs","hours to days"],
    timing: ["acute onset","progressive","waxing and waning"],
    worse: ["continued exposure","polypharmacy","alcohol combination"],
    better: ["decontamination","antidotes (if applicable)","supportive care","naloxone (opioids)"],
    assoc: ["altered mental status","respiratory depression","hypotension","hypothermia / hyperthermia","bradycardia / tachycardia","miosis / mydriasis","seizures","aspiration risk","track marks","paraphernalia present"],
    neg: ["no respiratory depression","no altered consciousness","no seizure","normal vitals","clear history"],
    riskFlags: { qual:["respiratory depression","altered consciousness","seizure"], msg:"⚠ TOXIDROME — establish airway, O₂, IV access, glucose, naloxone (if opioid), STAT labs (tox screen, ABG, lactate)" },
  },
    ...
    <div style={{ background: T.bg, minHeight: "100vh", fontFamily: "DM Sans, sans-serif", display: "flex", flexDirection: "column", position: "relative", overflow: "hidden", paddingTop: 40 }}>
      ...
      <div style={{ textAlign: "center", padding: "7px", borderTop: "1px solid rgba(26,53,85,0.3)", position: "relative", zIndex: 2 }}>
        <span style={{ fontFamily: "JetBrains Mono", fontSize: 9, color: T.txt4, letterSpacing: 2 }}>
          NOTRYA HPI · {CC_LIST.length} CHIEF COMPLAINTS · FLANK PAIN · WEAKNESS · COUGH · HEMOPTYSIS · ANAPHYLAXIS · INTOXICATION
        </span>
      </div>
    </div>
  );
