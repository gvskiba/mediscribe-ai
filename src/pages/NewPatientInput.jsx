...
export default function NewPatientInput() {
  ...
  // Extracted save handler — used by button AND keyboard shortcut
  const handleSaveChart = useCallback(async () => {
    try {
      const payload = {
        ...
        patient_gender:  demo.sex?.toLowerCase() === "male"   ? "male"
                       : demo.sex?.toLowerCase() === "female" ? "female" : "other",
        date_of_birth:   demo.dob  || "",
        chief_complaint: cc.text   || "",
        history_of_present_illness: cc.hpi || "",
        medications, allergies,
        status: "draft",
        registration_mrn:  registration.mrn  || "",
        registration_room: registration.room || "",
    ...
    pmhSelected, pmhExtra, surgHx, famHx, socHx,
    rosState, rosNotes, rosSymptoms, peState, peFindings,
    esiLevel, registration, navigate,
  ]);
  ...
  useEffect(() => {
    const handler = (e) => {
      ...
      // ⌘0–9 → jump to section
      if (mod && SHORTCUT_MAP[e.key]) {
        e.preventDefault();
        selectSection(SHORTCUT_MAP[e.key]);
        return;
      }
      // ⌘⇧E → open Note Studio with current patient data
      ...
      if (mod && e.shiftKey && e.key === "n") {
        e.preventDefault();
        navigate("/NewPatientInput?tab=demo");
        return;
      }
      ...
      {showShortcuts && (
        <div
          ...
          style={{
            ...
            background: "rgba(3,8,16,.75)", backdropFilter: "blur(4px)",
        ...
        >
          <div
            ...
            style={{
              ...
              borderRadius: 16, padding: "24px 28px", width: 460, maxWidth: "90vw",
          ...
          >
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 20 }}>
              ...
              <button
                onClick={() => setShowShortcuts(false)}
                style={{
                  background: "#0e2544", border: "1px solid #1a3555", borderRadius: 6,
                  width: 28, height: 28, color: "#7aa0c0", fontSize: 14, cursor: "pointer",
                  display: "flex", alignItems: "center", justifyContent: "center",
                }}
              >✕</button>
            </div>
            {[
              {
                section: "Navigate to section",
                rows: [
                  ["⌘ 1", "Demographics"],  ["⌘ 2", "Chief Complaint"],
                  ["⌘ 3", "Vitals"],        ["⌘ 4", "Meds & PMH"],
                  ["⌘ 5", "HPI"],           ["⌘ 6", "ROS"],
                  ["⌘ 7", "Physical Exam"], ["⌘ 8", "MDM"],
                  ["⌘ 9", "Orders"],        ["⌘ 0", "AutoCoder"],
                ],
              },
              {
                section: "Actions",
              ...
              <div key={section} style={{ marginBottom: 16 }}>
                <div style={{
                  fontFamily: "'JetBrains Mono',monospace", fontSize: 9, color: "#5a82a8",
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "6px 16px" }}>
                  {rows.map(([key, desc]) => (
                    <div key={key} style={{ display: "flex", alignItems: "center", gap: 8 }}>
                      <span style={{
                        fontFamily: "'JetBrains Mono',monospace", fontSize: 10, color: "#b8d4f0",
                        background: "#0e2544", border: "1px solid #1a3555", borderRadius: 4,
            ...
            <div style={{
              marginTop: 8, paddingTop: 12, borderTop: "1px solid #1a3555",
              fontFamily: "'JetBrains Mono',monospace", fontSize: 9,
              color: "#5a82a8", textAlign: "center",
            }}>
              press ? to dismiss
            </div>
          </div>
            ...
            {subItems.map((item) => (
              ...
              >
                ...
                {SECTION_SHORTCUT[item.section] && (
                  <span className="pill-sc">⌘{SECTION_SHORTCUT[item.section]}</span>
                )}
              </button>
            ))}
          </div>
