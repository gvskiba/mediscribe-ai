...
export default function QuickNote({ embedded = false, demo, vitals: initVitals, cc: initCC }) {
  ...
  // Print clinical inputs (HPI / ROS / PE) in clean paper format
  const printClinicalInputs = useCallback(() => {
...
<head>
  <meta charset="utf-8">
  <title>Clinical Documentation — ${now}</title>
  <style>
...
<body>
  <div class="page-header">
    <h1>Emergency Department Documentation</h1>
    <div class="meta-row">
      <div class="meta-field">
        <span class="meta-label">Patient</span>
        <span class="meta-value">${demo?.firstName || demo?.lastName
          ? [demo.firstName, demo.lastName].filter(Boolean).join(" ")
          : "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;"}</span>
      </div>
      <div class="meta-field">
        <span class="meta-label">MRN</span>
        <span class="meta-value">${demo?.mrn || "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;"}</span>
      </div>
      <div class="meta-field">
  ...
  <div class="section">
    ...
    <div class="section-body">${sec.text.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}</div>
  </div>`).join("")}
  ...
  <div class="footer">
    <span>Generated by Notrya QuickNote · AI-assisted documentation · Verify before charting</span>
    <span>${now}</span>
  </div>
...
Respond ONLY in valid JSON, no markdown fences.`;
  ...
  useEffect(() => {
    const fn = e => {
      ...
      // P — full page print · Shift+P — print clinical inputs (HPI/ROS/PE)
      if ((e.key === "p" || e.key === "P") && !e.ctrlKey && !e.metaKey) {
        e.preventDefault();
        if (e.shiftKey) { printClinicalInputs(); }
        else { window.print(); }
      }
    };
    window.addEventListener("keydown", fn);
    return () => window.removeEventListener("keydown", fn);
  }, [p2Open, mdmResult, dispResult, runMDM, runDisposition, copyNote, printClinicalInputs]);
    ...
    <div style={{ fontFamily:"'DM Sans',sans-serif",
      ...
      <div style={{ maxWidth:1100, margin:"0 auto",
        ...
        {!embedded && (
          <div style={{ padding:"18px 0 14px" }} className="no-print">
            ...
            <p style={{ fontFamily:"'DM Sans',sans-serif", fontSize:12,
              ...
              Paste · Cmd+Enter generate MDM · Complete workup · Cmd+Enter disposition · C copy · Ctrl+T template · Alt+H/R/E/L jump · Shift+P print inputs
            </p>
          </div>
        )}
        ...
        <div style={{ marginBottom:14,
          ...
          {/* Quick print — available as soon as any input exists */}
          {(hpi.trim() || ros.trim() || exam.trim()) && (
            <div style={{ display:"flex", justifyContent:"flex-end", marginBottom:8 }}
              className="no-print">
              <button onClick={printClinicalInputs}
                style={{ padding:"4px 12px", borderRadius:7, cursor:"pointer",
                  fontFamily:"'JetBrains Mono',monospace", fontSize:8, fontWeight:700,
                  border:"1px solid rgba(59,158,255,.35)", background:"rgba(59,158,255,.07)",
                  color:"var(--qn-blue)", letterSpacing:.5, textTransform:"uppercase",
                  transition:"all .15s" }}>
                🖨 Print HPI / ROS / PE  <span style={{ opacity:.5 }}>[Shift+P]</span>
              </button>
            </div>
          )}
          ...
          <div style={{ display:"flex", gap:8, flexWrap:"wrap",
            ...
            {(hpi.trim() || ros.trim() || exam.trim()) && (
              <button onClick={printClinicalInputs}
                style={{ padding:"7px 16px", borderRadius:7, cursor:"pointer",
                  fontFamily:"'DM Sans',sans-serif", fontWeight:600, fontSize:11,
                  border:"1px solid rgba(59,158,255,.35)", background:"rgba(59,158,255,.08)",
