          ...
          {!embedded&&(
            <div style={{marginBottom:10}} className="no-print">
              ...
              <div style={{display:"flex",gap:6,padding:"8px 10px",borderRadius:12,background:"rgba(8,22,40,.7)",border:"1px solid rgba(42,79,122,.35)"}}>
                ...
                <button onClick={()=>setShowCCLauncher(true)} title="Select Chief Complaint" style={{alignSelf:"center",padding:"5px 10px",borderRadius:7,cursor:"pointer",fontFamily:"'JetBrains Mono',monospace",fontSize:9,fontWeight:700,flexShrink:0,border:`1px solid ${ccProfile&&ccProfile.id!=="general"?(ccProfile.color||"var(--qn-teal)")+"50":"rgba(42,79,122,.4)"}`,background:ccProfile&&ccProfile.id!=="general"?(ccProfile.color||"var(--qn-teal)")+"10":"transparent",color:ccProfile&&ccProfile.id!=="general"?(ccProfile.color||"var(--qn-teal)"):"var(--qn-txt4)"}}>{ccProfile&&ccProfile.id!=="general"?`${ccProfile.icon} ${ccProfile.label}`:"⊞ Select CC"}</button>
