...
const FOLLOWUP = {
  rsi:["Confirm ETT position: ETCO₂ waveform ×6 + CXR (2–3 cm above carina)","Initiate sedation + analgesia immediately post-intubation — RASS -1 to 0","Post-intubation hypotension: fluid bolus + push-dose epinephrine. Rule out tension PTX, tamponade, auto-PEEP.","Daily SAT+SBT pairing from Day 1. Liberation assessment every day.","Document intubation in chart: indication, technique, medications, attempts, confirmation method"],
...
function RSICalc() {
  ...
  const groups = [
    ...
    {label:"Paralytics",color:T.coral,items:[["Succinylcholine 1.5",1.5,200,"45s onset · 10–15 min"],["Rocuronium 1.2",1.2,null,"60–90s onset · 45–70 min"],["Rocuronium 1.6 (RSI speed)",1.6,null,"Faster onset ≈ succinylcholine"]]},
        ...
        <div key={gi} style={{marginBottom:10}}>
          ...
          {g.items.map(([name,mg,mx,note],i)=>(
            <div key={i} style={{display:"flex",alignItems:"center",gap:8,background:"rgba(14,37,68,0.5)",border:`1px solid ${T.b}`,borderRadius:8,padding:"7px 12px",marginBottom:4}}>
              <div style={{flex:1}}><div style={{fontSize:11,fontWeight:600,color:T.txt}}>{name}</div><div style={{fontSize:9,color:T.txt3}}>{note}{mx?` · max ${mx}mg`:""}</div></div>
    ...
    <div style={{background:"rgba(0,229,192,0.05)",border:"1px solid rgba(0,229,192,0.22)",borderRadius:12,padding:"14px 16px",marginBottom:12}}>
      ...
      {validA && (
        <div>
          ...
          <div style={{marginTop:8,fontSize:9,color:T.txt4,fontFamily:"monospace"}}>Atropine 0.02 mg/kg IV (min 0.1 mg, max 0.5 mg) — prevents vagal bradycardia · Verify weight with Broselow tape</div>
        </div>
      )}
...
function RSIProcNote() {
  ...
  const generate=()=>{
    ...
    const wn=valid?` (${wKg.toFixed(1)} kg × ${ID[ia]} mg/kg)`:"";
    const pn=valid?` (${wKg.toFixed(1)} kg × ${PD[par]} mg/kg${PX[par]?`, max ${PX[par]}mg`:""})` :"";
