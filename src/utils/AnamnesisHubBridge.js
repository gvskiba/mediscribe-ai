// AnamnesisHubBridge.js
// Cross-hub data push layer for Lakonyx Anamnesis.
// Place at: @/utils/AnamnesisHubBridge.js
import { useState } from "react";
//
// When Anamnesis loads a patient's FHIR history, this module detects
// clinically relevant context for each Lakonyx hub and provides
// navigation functions that pre-populate hub context via URL params
// or the AnamnesisHubContext Base44 entity.
//
// Usage:
//   import { detectHubRelevance, navigateToHub } from "@/utils/AnamnesisHubBridge";
//   const relevance = detectHubRelevance(data, medNorm, ddiState);
//   navigateToHub("ecg", relevance.ecg);
...
function matchesPatterns(data={}, medNorm=[], patterns={}) {
  ...
  const probMatch = (data.problems??[]).some(p =>
    problems.some(pat => normStr(p.name).includes(pat)));

  const medPool = medNorm.length ? medNorm : (data.medications??[]);
  const medMatch = medPool.some(m =>
    meds.some(pat => normStr(m.ingredientName??m.cleanedName??m.name).includes(pat)));

  const visitMatch = (data.visits??[]).some(v =>
    visits.some(pat => normStr(v.cc).includes(pat) || normStr(v.dx).includes(pat)));

  return { matched: probMatch||medMatch||visitMatch, probMatch, medMatch, visitMatch };
}

// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
// CONTEXT BUILDERS — extract hub-specific clinical context
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
function buildECGContext(data, medNorm, ddiState) {
  ...
  const antiarrhythmics = meds.filter(m =>
    ["amiodarone","sotalol","dofetilide","flecainide","propafenone","dronedarone","digoxin"].some(d=>normN(m).includes(d))
  ).map(m => m.name);

  const betablockers = meds.filter(m =>
    ["metoprolol","atenolol","carvedilol","bisoprolol","propranolol","labetalol"].some(d=>normN(m).includes(d))
  ).map(m => m.name);
  ...
  return {
    ...
    summary: [
      priorAFib&&"Prior AFib", priorMI&&"Prior MI/STEMI", priorHF&&"Heart failure",
      pacerICD&&"Pacemaker/ICD", qtDDI&&"QT-prolonging DDI",
      antiarrhythmics.length&&`On antiarrhythmics: ${antiarrhythmics.slice(0,2).join(", ")}`,
    ].filter(Boolean).join(" · ") || "Cardiac history present",
  };
}
...
function buildStrokeContext(data, medNorm) {
  ...
  return {
    priorStroke, priorTIA, afib, htn, dm, carotid, onAnticoag,
    anticoag, antiplatelet,
    cha2ds2Score: [afib,priorStroke||priorTIA,htn,dm,(data.patient?.dob&&new Date().getFullYear()-parseInt(data.patient.dob)>=75),
    ].filter(Boolean).length,
    lastStrokeVisit: priorStrokeVisit?.date??null,
    summary: [
      priorStroke&&"Prior stroke/CVA", priorTIA&&"Prior TIA",
      afib&&"Known AFib", onAnticoag&&`Anticoagulated: ${anticoag.slice(0,2).join(", ")}`,
      carotid&&"Carotid disease",
    ].filter(Boolean).join(" · ") || "Stroke risk factors present",
  };
}
...
function buildToxContext(data, medNorm, visits=[]) {
  ...
  const matTx     = matp.length > 0;

  return {
    matMeds:matp, naloxone, opioids, benzos,
    odHistory: odVisits.length > 0,
    odCount: odVisits.length,
    etohHistory: etohVisits.length > 0,
    sudDiagnosis: sudProb,
    matTreatment: matTx,
    opioidBenzoCombination: opioids.length>0 && benzos.length>0,
    summary: [
      odVisits.length>0&&`${odVisits.length} prior OD visit${odVisits.length>1?"s":""}`,
      matTx&&`MAT: ${matp.slice(0,2).join(", ")}`,
      naloxone&&"On naloxone",
      opioids.length>0&&benzos.length>0&&"⚠ Opioid + benzo combo",
    ].filter(Boolean).join(" · ") || "Substance-related history present",
  };
}
...
function buildCardiacRiskContext(data, medNorm) {
  ...
  const timi        = [dm,htn,hld,smoking,priorMI||priorCAD,dm].filter(Boolean).length;

  return {
    dm, htn, hld, smoking, obesity, priorMI, priorCAD, ckd,
    statins, timi,
    riskFactorCount: [dm,htn,hld,smoking,obesity,priorMI,priorCAD].filter(Boolean).length,
    summary: [
      dm&&"DM", htn&&"HTN", hld&&"Hyperlipidemia", smoking&&"Tobacco",
      priorMI&&"Prior MI", priorCAD&&"CAD", ckd&&"CKD",
      statins.length>0&&`On statin: ${statins[0]}`,
    ].filter(Boolean).join(" · ") || "Cardiac risk factors present",
  };
}

// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
// MAIN EXPORT — detectHubRelevance
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
// NAVIGATION — open a hub with pre-populated patient context
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
// Build URL params from patient seed + hub context for deep-linking
export function buildHubParams(patient={}, context={}) {
  ...
  // Encode context summary for the hub to display
...
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
// HUB BUTTONS COMPONENT — renders relevant hub deep-links in Anamnesis footer
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
export function HubBridgePanel({ data, medNorm, ddiState }) {
  ...
  return (
    <div>
      <div style={{ fontSize:9, fontWeight:700, color:"#3a5f80", letterSpacing:"0.10em",
        textTransform:"uppercase", marginBottom:6 }}>Open in Lakonyx Hub</div>
